...
//“双摇杆+键盘”三合一输入组件——左侧摇杆/ADWS 控制移动，右侧摇杆控制瞄准射击，所有输入统一转成方向与力度事件广播给角色，松手即自动归零。
...
export class Joystick extends Component {
    ...
    OnTouchStart(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            // 允许多个触摸同时进行，不检查 _movementTouch 是否存在
    ...
    OnTouchMove(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                ...
                CDXX2_EventManager.Scene.emit(CDXX2_MyEvent.CDXX2_MOVEMENT, dirX, dirY, len / radius);
            }
        }
    }
    ...
    OnTouchEnd(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                ...
                CDXX2_EventManager.Scene.emit(CDXX2_MyEvent.CDXX2_MOVEMENT, 0, 0, 0);
    ...
    OnAttackTouchMove(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._attackMovementTouch && touch.getID() == this._attackMovementTouch.getID()) {
                ...
                CDXX2_EventManager.Scene.emit(CDXX2_MyEvent.CDXX2_ATTACK_START, dirX, dirY);
            }
        }
    }
    ...
    OnAttackTouchEnd(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._attackMovementTouch && touch.getID() == this._attackMovementTouch.getID()) {
                ...
                CDXX2_EventManager.Scene.emit(CDXX2_MyEvent.CDXX2_ATTACK_END);
            }
        }
    }

    // onReloadTouchEnd(event: EventTouch) {
    //     CDXX2_EventManager.Scene.emit(CDXX2_MyEvent.CDXX2_JUMP);
    // }
    ...
    updateDir() {
        ...
        CDXX2_EventManager.Scene.emit(CDXX2_MyEvent.CDXX2_MOVEMENT, this._dir.x, this._dir.y, 1.0);
    }
    //#endregion
