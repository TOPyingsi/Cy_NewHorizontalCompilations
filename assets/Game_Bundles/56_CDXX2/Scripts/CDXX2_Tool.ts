...
@ccclass('CDXX2_Tool')
export class CDXX2_Tool extends Component {

    /*
    静态工具箱：
    用中文单位（万/亿/万亿/兆等）格式化超大数字；
    提供深度拷贝、数组乱序、根据枚举值反查键名三个常用方法。
    */
    ...
    public static formatNumber(number: number, fixed: number = 1) {
        if (number >= 1e24) {
            // 大于或等于 1万垓，用“姊”作单位
            return (number / 1e24).toFixed(fixed) + "姊";
        } else if (number >= 1e20) {
            // 大于或等于 1 万京，用“垓”作单位
            return (number / 1e20).toFixed(fixed) + "垓";
        } else if (number >= 1e16) {
            // 大于或等于 1万兆，用“京”作单位
            return (number / 1e16).toFixed(fixed) + "京";
        } else if (number >= 1e12) {
            // 大于或等于 1 万亿，用“兆”作单位
            return (number / 1e12).toFixed(fixed) + "兆";
        } else if (number >= 1e8) {
            // 大于或等于 1 亿，用“亿”作单位
