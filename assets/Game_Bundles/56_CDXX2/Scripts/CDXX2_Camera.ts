...
//“摄像机”在非战斗阶段平滑跟随玩家，并限制在指定矩形范围内；一旦进入战斗立即切到固定俯视位，Y 轴始终保持初始高度。

@ccclass('CDXX2_Camera')
export class CDXX2_Camera extends Component {
    ...
    protected update(dt: number): void {
        if (CDXX2_GameManager.Instance.IsBattle) {
        ...
        if (this.Target) {
            ...
            // 限制摄像机的移动范围
            smoothPos.x = math.clamp(smoothPos.x, this.cameraBounds.xMin - 628, this.cameraBounds.xMax);
            // Y 轴不限制，完全跟随角色运动
