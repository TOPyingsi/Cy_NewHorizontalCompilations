...
@ccclass('TaiKongWaJue_UpgradePanel')
export class TaiKongWaJue_UpgradePanel extends Component {
    ...
    Upgrade(event: Event) {
        ...
        if (coin >= need) {
            ...
            this.Init();
            TaiKongWaJue_Audio.Instance.PlayAudio("sell");
    ...
    Charge() {
        ...
        if (coin >= 1) {
            coin -= 1;
            localStorage.setItem("DAHCV_Coins", coin.toString());
            TaiKongWaJue_PlayerController.Instance.elc = 10 * (1 + parseInt(localStorage.getItem("DAHCV_Elc")));
