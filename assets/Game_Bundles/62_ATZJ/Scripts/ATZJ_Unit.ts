...
import { ATZJ_AttackBox } from './ATZJ_AttackBox';
import { ATZJ_GameManager } from './ATZJ_GameManager';
import { ATZJ_Bullet } from './ATZJ_Bullet';
...
import { ATZJ_TongLing } from './ATZJ_TongLing';
const { ccclass, property } = _decorator;

@ccclass('ATZJ_Unit')
export class ATZJ_Unit extends Component {
    ...
    //读取角色数据
    LoadUnitData() {
        let data = ATZJ_Constant.GetUnitDataByName(this.node.name);
    ...
    SkillClick(num: number): void {
        ...
        this.Mp -= num * 20;//MP消耗
        director.getScene().emit(ATZJ_EasyControllerEvent.释放技能);
    ...
    TongLing(num: number) {
        ...
        director.getScene().emit(ATZJ_EasyControllerEvent.消耗技能豆);
        ATZJ_incident.Loadprefab("PreFab/通灵/" + num).then((pre: Prefab) => {
            let TL = instantiate(pre);
            TL.setParent(ATZJ_GameManager.Instance.BG);
            TL.setWorldPosition(this.node.worldPosition.clone());
            TL.getComponent(ATZJ_TongLing).Init(this.node, this.Attack, !this.orientation);
        });
    }
    ...
    AttackCollisionLook(name: string) {
        ...
        collider.getComponent(ATZJ_AttackBox).Attack = this.Attack * this.GetSkillScale(name);
        collider.getComponent(ATZJ_AttackBox).Attacknode = this.node;
        collider.getComponent(ATZJ_AttackBox).Show();
    ...
    Hurt(attack: number): boolean {
        ...
        this.Hp -= attack;
        this.Mp += 3;
        director.getScene().emit(ATZJ_EasyControllerEvent.BeatBack, this.node, attack);
        ...
        ATZJ_incident.Loadprefab("PreFab/击中特效").then((pre: Prefab) => {
    ...
    private handleHitEffect() {
        ...
        // 播放受击动画
        this.aniName = "受击";
        director.getScene().emit(ATZJ_EasyControllerEvent.受击, this._comboCount);
        // 判断是否浮空
    ...
    die() {
        ...
        if (this.node.getComponent(ATZJ_AIControl)) this.node.getComponent(ATZJ_AIControl).enabled = false;
        if (this.node.getComponent(ATZJ_PlayerControl)) this.node.getComponent(ATZJ_PlayerControl).enabled = false;
        this.StopMove();
        // 1秒后销毁节点
        this.scheduleOnce(() => {
            director.getScene().emit(ATZJ_EasyControllerEvent.角色死亡, this.node.getComponent(ATZJ_AIControl) ? true : false);
    ...
    GenerateBullet(id: number, pos: Vec3, attack: number) {
        ...
        nd.active = false;
        nd.setParent(ATZJ_GameManager.Instance.BG);
        nd.setWorldPosition(this.node.getWorldPosition().clone().add(pos));
        nd.active = true;
        nd.getComponent(ATZJ_Bullet).Init(this.node, attack, !this.orientation);
    }
    //角色位移
    MovePos(pos: Vec2, Time: number = 0.1) {
        ...
        if (pos.y > ATZJ_GameManager.Instance.MapData.x) pos.y = ATZJ_GameManager.Instance.MapData.x;
        if (pos.y < ATZJ_GameManager.Instance.MapData.y) pos.y = ATZJ_GameManager.Instance.MapData.y;
        if (pos.x < ATZJ_GameManager.Instance.MapData.z) pos.x = ATZJ_GameManager.Instance.MapData.z;
        if (pos.x > ATZJ_GameManager.Instance.MapData.w) pos.x = ATZJ_GameManager.Instance.MapData.w;
