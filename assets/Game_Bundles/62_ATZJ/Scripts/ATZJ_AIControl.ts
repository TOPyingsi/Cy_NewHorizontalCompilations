...
@ccclass('ATZJ_AIControl')
export class ATZJ_AIControl extends Component {
    ...
    start() {
        ...
        // 初始化最后玩家位置
        if (ATZJ_GameManager.Instance && ATZJ_GameManager.Instance.PlayerNode) {
            this.lastPlayerPosition = ATZJ_GameManager.Instance.PlayerNode.worldPosition.clone();
        }
        // 初始化战术状态
    ...
    update(dt: number) {
        this.Startforbid -= dt;
        if (!ATZJ_GameManager.Instance || !ATZJ_GameManager.Instance.PlayerNode || this.Startforbid > 0) {
            return;
        }
    ...
    private makeDecision() {
        const playerNode = ATZJ_GameManager.Instance.PlayerNode;
    ...
    private predictPlayerPosition(playerPosition: Vec3, playerNode: Node): Vec3 {
        const playerUnit = playerNode.getComponent(ATZJ_Unit);
    ...
    private executeFlankingBehavior(predictedPlayerPosition: Vec3, distanceToPlayer: number) {
        ...
        if (distanceToPlayer > this.attackRange * 2) {
            ...
            if (this.unit.SkillCount > 0) {
                this.unit.TongLing(ATZJ_GameManager.SkillData[ATZJ_GameManager.EnemyID])
                return;
            }
            if (currentTime - this.lastAttackTime > this.attackCooldown) {
                this.unit.AttackClick();
