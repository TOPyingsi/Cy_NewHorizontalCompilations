...
@ccclass('MTRNX_Water_WBullet3')
export class MTRNX_Water_WBullet3 extends Component {
    ...
    update(deltaTime: number) {
        ...
        if (this.target?.getComponent(MTRNX_Water_Unit) != null && this.target.getComponent(MTRNX_Water_Unit).Hp > 0) {
            var dir = this.target.getWorldPosition().subtract(this.node.getWorldPosition()).normalize();
    ...
    //AnimationOver
    AnimationOver() {
        this.scheduleOnce(() => { MTRNX_Water_PoolManager.Instance.PutNode(this.node); });
    }
}
