...
import { MTRNX_Water_UIManager } from './MTRNX_Water_UIManager';
import { MTRNX_Water_BloodBar_Mtr } from './UI/MTRNX_Water_BloodBar_Mtr';
...
import { MTRNX_Water_BloodLabel_Mtr } from './UI/MTRNX_Water_BloodLabel_Mtr';
import { MTRNX_Water_GameDate } from './MTRNX_Water_GameDate';
import { MTRNX_Water_EasyController, MTRNX_Water_EasyControllerEvent } from './MTRNX_Water_EasyController';
import { UIManager } from '../../../Scripts/Framework/Managers/UIManager';
const { ccclass, property } = _decorator;
const { Ray } = geometry;
const outRay = new geometry.Ray();

@ccclass('MTRNX_Water_Unit')
export class MTRNX_Water_Unit extends Component {
    ...
    public SubMp(num: number): boolean {
        if (this.Mp < num) {
            MTRNX_Water_UIManager.HopHint("能量不足，无法施放技能！")
            return false;
        } else {
            this.Mp -= num;
            this.node.getChildByName("BloodBar")?.getComponent(MTRNX_Water_BloodBar_Mtr).SetBlueBar(this.Mp / this.maxMp);
            return true;
        }
    }
    ...
    start() {
        ...
        this.circlecollider.on(Contact2DType.BEGIN_CONTACT, (selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) => {
            if (otherCollider.node.getComponent(MTRNX_Water_Unit) && this.IsEnemy != otherCollider.node.getComponent(MTRNX_Water_Unit).IsEnemy
                && otherCollider.node.getComponent(MTRNX_Water_Unit).State != 2 && this.TargetNodes.indexOf(otherCollider.node) == -1) {//检测单位是否为敌方且检测单位为非死亡状态
        ...
        this.circlecollider.on(Contact2DType.END_CONTACT, (selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) => {
            if (otherCollider.node.getComponent(MTRNX_Water_Unit) && this.IsEnemy != otherCollider.node.getComponent(MTRNX_Water_Unit).IsEnemy && this.TargetNodes.indexOf(otherCollider.node) != -1) {
            ...
            if (otherCollider.node.name == "黑色") {
                ...
                if (this.IsEnemy) {
                    ...
                    if (otherCollider.node.name == "我方单位") {
                        MTRNX_Water_GameManager.Instance.RedHP -= Math.ceil((MTRNX_Water_Constant.MTTypePointCost[this.Id] / 100));
                    ...
                    if (otherCollider.node.name == "敌方单位") {
                        if (MTRNX_Water_GameManager.GameMode == MTRNX_Water_GameMode.Endless) {
                            MTRNX_Water_GameManager.Instance.Score += Math.floor(MTRNX_Water_Constant.JKTypePointCost[this.Id] / 5);
                        } else {
                            if (MTRNX_Water_JKType[this.Id] == "激光监控人") {
                                MTRNX_Water_GameManager.Instance.BlueHP -= 1;
                            } else {
                                MTRNX_Water_GameManager.Instance.BlueHP -= Math.ceil((MTRNX_Water_Constant.JKTypePointCost[this.Id] / 100));
                            }
                        }
                        this.scheduleOnce(() => { this.node.destroy(); });
                        MTRNX_Water_GameManager.Instance.ShakeWhite();
        ...
        BundleManager.GetBundle("56_MTRNX_Water_Bundle").load("Prefabs/UI/BloodBar", Prefab, (err, res) => {
            if (err) {
                console.log(err);
                return;
            }
            let blood = MTRNX_Water_PoolManager.Instance.GetNode(res, this.node);
            blood.getComponent(MTRNX_Water_BloodBar_Mtr).Init(this.bloodBarScale);
        })
    ...
    InitMassacreEvent() {
        if (this.IsMassacreUnit) {
            //杀戮模式操作监听
            MTRNX_Water_EasyController.on(MTRNX_Water_EasyControllerEvent.MOVEMENT, this.onMovement, this);
            MTRNX_Water_EasyController.on(MTRNX_Water_EasyControllerEvent.MOVEMENT_STOP, this.onMovementRelease, this);
            MTRNX_Water_EasyController.on(MTRNX_Water_EasyControllerEvent.JUMP, this.onJump, this);
            for (let i of this.Btns) {
                MTRNX_Water_GameManager.Instance.UI.getChildByName(i).on(Node.EventType.TOUCH_END, this.ButtonClicked, this);
            }
        }
    }
    ...
    ButtonClicked(event: EventTouch) {
        if (this.ZunitState() == 0) {
            switch (event.target.name) {
                ...
                case "技能一":
                    {
                        this.canShoot = this.SubMp(MTRNX_Water_Constant.PlayerMpconsume[MTRNX_Water_GameDate.Instance.CurrentSelect][2]);
                        this.node.setScale(Vec3.ONE);
                    }
                    break;
                case "技能二":
                    {
                        this.canSkill = this.SubMp(MTRNX_Water_Constant.PlayerMpconsume[MTRNX_Water_GameDate.Instance.CurrentSelect][3]);
                        this.node.setScale(Vec3.ONE);
                    }
                    break;
                case "技能三":
                    {
                        this.canSkill2 = this.SubMp(MTRNX_Water_Constant.PlayerMpconsume[MTRNX_Water_GameDate.Instance.CurrentSelect][4]);
                        this.node.setScale(Vec3.ONE);
                    }
                    break;
                case "技能四":
                    {
                        this.canSkill3 = this.SubMp(MTRNX_Water_Constant.PlayerMpconsume[MTRNX_Water_GameDate.Instance.CurrentSelect][5]);
                        this.node.setScale(Vec3.ONE);
                    }
                    break;
            }
    ...
    onJump() {
        if (!this.IsInTheAir) {
            ...
            let force = 1200000;
            switch (MTRNX_Water_GameManager.Instance.SelectUnit) {
    ...
    ChangeUnitScale() {
        ...
        this.node.getChildByName("BloodBar")?.getComponent(MTRNX_Water_BloodBar_Mtr).ResetScale();//人物翻转的同时血条也需要翻转
    ...
    Attackincident() {
        ...
        if (this.TargetNodes.length > 0) {
            if (this.IsSingleAtk) {
                this.TargetNodes[0].getComponent(MTRNX_Water_Unit).Hurt(this.attack);
            }
            else {
                for (let i of this.TargetNodes) {
                    i.getComponent(MTRNX_Water_Unit).Hurt(this.attack);
                }
            }
        }
        this.ChangeUnitScale();
    ...
    Hurt(atk: number) {
        if (this.State == 2) return;
        this.Hp -= atk;
        this.node.getChildByName("BloodBar")?.getComponent(MTRNX_Water_BloodBar_Mtr).SetBar(this.Hp / this.maxHp);
        try {
            this._rigibody2D.linearVelocity = v2(0, this._rigibody2D.linearVelocity.y);
        } catch (error) {
        }
        ...
        MTRNX_Water_AudioManager.AudioClipPlay("受击");

        BundleManager.GetBundle("56_MTRNX_Water_Bundle").load("Prefabs/UI/BloodLabel", Prefab, (err, res) => {
            if (err) {
                console.log(err);
                return;
            }
            let bloodlabel: Node = MTRNX_Water_PoolManager.Instance.GetNode(res, this.node);
            bloodlabel.getComponent(MTRNX_Water_BloodLabel_Mtr).Init(atk);
        })
    }
    ...
    Dieincident() {
        ...
        //无尽模式加分
        if (MTRNX_Water_GameManager.GameMode == MTRNX_Water_GameMode.Endless && this.IsEnemy == true) {
            MTRNX_Water_GameManager.Instance.Score += Math.floor(MTRNX_Water_Constant.MTTypePointCost[this.Id] / 5);
        }
        //杀戮模式加分
        if (MTRNX_Water_GameManager.GameMode == MTRNX_Water_GameMode.Massacre && this.IsEnemy == true) {
            MTRNX_Water_GameManager.Instance.Score += Math.floor(MTRNX_Water_Constant.MTTypePointCost[this.Id] / 5 * MTRNX_Water_GameManager.Gamedifficulty);
        }
    }
    ...
    secondupdate() {
        ...
        if (this.StateTime[1] > 0) {//回蓝
            ...
            this.node.getChildByPath("BloodBar")?.getComponent(MTRNX_Water_BloodBar_Mtr).SetBlueBar(this.Mp / this.maxMp);
        }
