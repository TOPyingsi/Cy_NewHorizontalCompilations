...
@ccclass('MTRNX_Water_ZAniEvent')
export class MTRNX_Water_ZAniEvent extends Component {

    self: MTRNX_Water_ZUnit;
    start() {
        this.self = this.node.parent.getComponent(MTRNX_Water_ZUnit);
    }
    ...
    Hit(num1: number) {
        if (MTRNX_Water_GameManager.GameMode == MTRNX_Water_GameMode.Massacre && this.node.parent.getComponent(MTRNX_Water_Unit).IsEnemy == false) {
            let attack = MTRNX_Water_GameManager.Instance.MassacreUnit.getComponent(MTRNX_Water_Unit).attack;
    ...
    Shoot(num: number, damage: number, speed: number, traceLevel: number, dispersion: number) {
        ...
        var bullet = MTRNX_Water_PoolManager.Instance.GetNode(this.self.bullets[num], MTRNX_Water_GameManager.Instance.GameNode.getChildByName("子弹层")) as Node;
        ...
        if (MTRNX_Water_GameManager.GameMode == MTRNX_Water_GameMode.Massacre) {
            attack = MTRNX_Water_GameManager.Instance.MassacreUnit.getComponent(MTRNX_Water_Unit).attack;
        }
        ...
        if (src) {
            ...
            bullet.getComponent(MTRNX_Water_Gbullet).init(this.self.node.getChildByName("ShootPoint").getWorldPosition().add(v3(1, 0, 0).multiplyScalar(this.self.IsEnemy ? -1 : 1)), this.self.forward.clone(), damage, speed, this.self.IsEnemy, true, 1, true, false);
        }
    }
    ...
    DieEnd() {
        if (!this.self.boom)
            ...
            var boom = MTRNX_Water_PoolManager.Instance.GetNode(this.self.boom, MTRNX_Water_GameManager.Instance.GameNode);
    ...
    AudioPlay(name: string) {
        MTRNX_Water_AudioManager.AudioClipPlay(name);
    }
    //向前冲刺一段距离
