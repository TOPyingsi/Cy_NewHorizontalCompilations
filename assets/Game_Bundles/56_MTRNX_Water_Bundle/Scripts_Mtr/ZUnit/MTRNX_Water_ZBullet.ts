...
@ccclass('MTRNX_Water_ZBullet')
export class MTRNX_Water_ZBullet extends Component {
    ...
    private Shoot() {
        if (!this.target || this.traceLevel == 0) var dir = v3(this.isEnemy ? -1 : 1, 0);
            ...
            if (this.target.getComponent(MTRNX_Water_ZUnit)) var dir = this.target.getWorldPosition().subtract(this.node.getWorldPosition()).normalize();
        ...
        if (this.traceLevel == 1) {
            let dispersion = randomRange(-this.dispersion, this.dispersion);
            degree = degree + dispersion;
            Vec3.rotateZ(dir, dir, Vec3.ZERO, toRadian(dispersion));
        }
