...
import { MTRNX_Water_ZArmedUnit } from './MTRNX_Water_ZArmedUnit';
...
@ccclass('MTRNX_Water_ZWeapon')
export class MTRNX_Water_ZWeapon extends Component {
    ...
    cd = 0;
    self: MTRNX_Water_ZArmedUnit;
    ani: Animation;

    start() {
        this.self = this.node.parent.parent.getComponent(MTRNX_Water_ZArmedUnit);
    ...
    Shoot() {
        ...
        var bullet = MTRNX_Water_PoolManager.Instance.GetNode(this.self.bullets[this.self.weapons.children.indexOf(this.node)], MTRNX_Water_GameManager.Instance.GameNode.getChildByName("子弹层")) as Node;
        ...
        if (src) {
            ...
            bullet.getComponent(MTRNX_Water_Gbullet).init(this.node.getChildByName("ShootPoint").getWorldPosition().add(v3(1, 0, 0).multiplyScalar(this.self.IsEnemy ? -1 : 1)), this.self.forward.clone(), this.damage, this.speed, this.self.IsEnemy, true, 1, true, false);
        }
    }
