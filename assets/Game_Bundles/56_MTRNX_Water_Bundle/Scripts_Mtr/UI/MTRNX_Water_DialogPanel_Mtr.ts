...
@ccclass('MTRNX_Water_DialogPanel_Mtr')
export class MTRNX_Water_DialogPanel_Mtr extends Component {
    ...
    Show(data: any, cb: Function = null) {
        ...
        // BundleManager.LoadSpriteFrame("56_MTRNX_Water_Bundle", `Icons/Speaker_${this.dialogData.Speaker}`).then((sp: SpriteFrame) => {
        ...
        switch (event.target.name) {
            case 'Mask':
                ...
                if (this.index >= this.dialogData.Desc.length) {
                    MTRNX_Water_UIManager.Instance.HidePanel(MTRNX_Water_Panel.DialogPanel);
                    this.nowText = null;
                    this.cb && this.cb();
                    return;
                }
