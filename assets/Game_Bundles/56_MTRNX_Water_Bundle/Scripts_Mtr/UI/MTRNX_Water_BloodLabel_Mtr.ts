...
@ccclass('MTRNX_Water_BloodLabel_Mtr')
export class MTRNX_Water_BloodLabel_Mtr extends Component {

    Init(str: number) {
        this.node.setScale(this.node.parent.scale.x, 1, 1);
        if (this.node.parent.getComponent("MTRNX_Water_ZUnit")) this.node.setPosition(0, this.node.parent.getComponent(BoxCollider2D).size.height / 2);
