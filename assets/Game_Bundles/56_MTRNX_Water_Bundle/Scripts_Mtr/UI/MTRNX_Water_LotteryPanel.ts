...
@ccclass('MTRNX_Water_LotteryPanel')
export class MTRNX_Water_LotteryPanel extends Component {
    ...
    protected start(): void {
        this.Maxweight = MTRNX_Water_LotteryPanel.Data.reduce((a, b) => { return { Name: "", weight: a.weight + b.weight } }).weight;
    }
    ...
    //按下抽奖按钮
    OnlotteryClick() {
        if (MTRNX_Water_Include.GetPointIsCan(1000)) {
            MTRNX_Water_Include.AddPoint(-1000, false);
            this.lottery();
        }
    }
    ...
    lottery() {
        ...
        let nownum: number = 0;
        let award: string = MTRNX_Water_LotteryPanel.Data[0].Name;
        for (let i = 0; i < MTRNX_Water_LotteryPanel.Data.length; i++) {
            nownum += MTRNX_Water_LotteryPanel.Data[i].weight;
            if (nownum > num) {
                award = MTRNX_Water_LotteryPanel.Data[i].Name;
                break;
            }
        }
        switch (award) {
            case "科技点*300": MTRNX_Water_Include.AddPoint(300); break;
            case "科技点*500": MTRNX_Water_Include.AddPoint(500); break;
            case "科技点*800": MTRNX_Water_Include.AddPoint(800); break;
            case "科技点*1200": MTRNX_Water_Include.AddPoint(1200); break;
            case "角色碎片*20": MTRNX_Water_Include.AddDebris(20); break;
            case "角色碎片*40": MTRNX_Water_Include.AddDebris(40); break;
            case "角色碎片*60": MTRNX_Water_Include.AddDebris(60); break;
            default: MTRNX_Water_Include.UnlookPlayer(award); break;
        }
    }
    ...
    //关闭
    OnExitClick() {
        MTRNX_Water_UIManager.Instance.HidePanel(MTRNX_Water_Panel.LotteryPanel);
    }
}
