...
@ccclass('MTRNX_Water_SuperShop')
export class MTRNX_Water_SuperShop extends Component {
    @property(Node)
    Content: Node = null;
    ...
    //返回
    OnExitClick() {
        MTRNX_Water_UIManager.Instance.HidePanel(MTRNX_Water_Panel.SuperShop);
    }
    ...
    OnbuttonClick(btn: EventTouch) {
        switch (btn.target.name) {
            ...
            case "切换":
                ...
                break;
            case "碎片解锁":
                this.Buy_Debris(btn.target.parent.getComponent(MTRNX_Water_Shop));
                break;
        }
    ...
    Buy(shopdata: MTRNX_Water_Shop) {
        if (MTRNX_Water_GameDate.Instance.PlayerDate[shopdata.id] == 0) {
            if (MTRNX_Water_GameDate.Instance.Money >= shopdata.Point_price) {
                MTRNX_Water_GameDate.Instance.PlayerDate[shopdata.id] = 1;
                MTRNX_Water_Include.AddPoint(-shopdata.Point_price, false);
                this.Show();
            } else {
                MTRNX_Water_UIManager.HopHint("科技点数不足！需要科技点*" + shopdata.Point_price);
            }
        } else {
    ...
    //碎片解锁
    Buy_Debris(shopdata: MTRNX_Water_Shop) {
        if (MTRNX_Water_GameDate.Instance.PlayerDate[shopdata.id] == 0) {
