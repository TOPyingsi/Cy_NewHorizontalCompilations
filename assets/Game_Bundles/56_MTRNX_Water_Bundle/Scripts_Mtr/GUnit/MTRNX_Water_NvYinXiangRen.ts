...
@ccclass('MTRNX_Water_NvYinXiangRen')
export class MTRNX_Water_NvYinXiangRen extends MTRNX_Water_Unit {
    ...
    start(): void {
        ...
        this.node.getComponents(CircleCollider2D)[1].on(Contact2DType.BEGIN_CONTACT, (selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) => {
            if (otherCollider.node.getComponent(MTRNX_Water_Unit) && this.IsEnemy != otherCollider.node.getComponent(MTRNX_Water_Unit).IsEnemy
                && otherCollider.node.getComponent(MTRNX_Water_Unit).State != 2 && this.SkillTargets.indexOf(otherCollider.node) == -1) {//检测单位是否为敌方且检测单位为非死亡状态
        ...
        this.node.getComponents(CircleCollider2D)[1].on(Contact2DType.END_CONTACT, (selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) => {
            if (otherCollider.node.getComponent(MTRNX_Water_Unit) && this.IsEnemy != otherCollider.node.getComponent(MTRNX_Water_Unit).IsEnemy && this.SkillTargets.indexOf(otherCollider.node) != -1) {
    ...
    Attackincident() {
        ...
        if (this.AttckNum > 0) {
            if (this.SkillTargets.length > 0) {
                MTRNX_Water_AudioManager.AudioClipPlay("刀攻击");
                this.SkillTargets[0].getComponent(MTRNX_Water_Unit).Hurt(this.attack);
            }
        }
        else {
            if (this.TargetNodes.length > 0) {
                ...
                MTRNX_Water_ResourceUtil.LoadPrefab("Bullet/GFlyKnife").then((prefab: Prefab) => {
                    ...
                    bullet.getComponent(MTRNX_Water_Gbullet).init(targetPoint, this.forward.clone(), this.attack / 2, 40, this.IsEnemy, true, 1, false, false);
                });
            }
        }
    }
