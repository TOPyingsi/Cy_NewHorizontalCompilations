...
import { MTRNX_Water_GEffect } from './MTRNX_Water_GEffect';
import { MTRNX_Water_ZUnit } from '../ZUnit/MTRNX_Water_ZUnit';

const { ccclass, property } = _decorator;

@ccclass('MTRNX_Water_Gbullet')
export class MTRNX_Water_Gbullet extends Component {
    ...
    onBeginContact(selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
        ...
        if (other.name == "地面" || other.name == "黑色") MTRNX_Water_PoolManager.Instance.PutNode(this.node);
        else if (other.getComponent(MTRNX_Water_Unit)) {
            ...
            if (this.node.name == "GFlyKnife") {
                if (other.getComponent(MTRNX_Water_ZUnit)) {
                ...
                MTRNX_Water_ResourceUtil.LoadPrefab("Bullet/GLight").then((prefab: Prefab) => {
                    let effect: Node = MTRNX_Water_PoolManager.Instance.GetNode(prefab, MTRNX_Water_GameManager.Instance.GameNode.getChildByName("子弹层"));
                    effect.getComponent(MTRNX_Water_GEffect).Init(this.posed, 0.85);
                    MTRNX_Water_AudioManager.AudioClipPlay("音响攻击音效");
                })
            }
            else {
                this.posed.set(this.node.worldPosition);
                MTRNX_Water_ResourceUtil.LoadPrefab("Bullet/Geffect").then((prefab: Prefab) => {
                    let effect: Node = MTRNX_Water_PoolManager.Instance.GetNode(prefab, MTRNX_Water_GameManager.Instance.GameNode.getChildByName("子弹层"));
                    effect.getComponent(MTRNX_Water_GEffect).Init(this.posed, 0.4);
                })
            }
        }
    }
