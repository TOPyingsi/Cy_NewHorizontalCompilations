...
@ccclass('MTRNX_Water_WuShuangZhanShen2')
export class MTRNX_Water_WuShuangZhanShen2 extends MTRNX_Water_Unit {
    ...
    start() {
        this.node.getChildByName("攻击区域").getComponent(BoxCollider2D).on(Contact2DType.BEGIN_CONTACT, (selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) => {
            if (otherCollider.node.getComponent(MTRNX_Water_Unit) && this.IsEnemy != otherCollider.node.getComponent(MTRNX_Water_Unit).IsEnemy
                && otherCollider.node.getComponent(MTRNX_Water_Unit).State != 2) {//检测单位是否为敌方且检测单位为非死亡状态
                otherCollider.node.getComponent(MTRNX_Water_Unit).Hurt(this.attack);
            }
        })
