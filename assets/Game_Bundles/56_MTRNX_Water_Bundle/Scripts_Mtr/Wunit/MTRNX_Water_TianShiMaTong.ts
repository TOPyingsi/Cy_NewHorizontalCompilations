...
@ccclass('MTRNX_Water_TianShiMaTong')
export class MTRNX_Water_TianShiMaTong extends MTRNX_Water_Unit {
    ...
    start() {
        ...
        this.node.getComponents(CircleCollider2D)[1].on(Contact2DType.BEGIN_CONTACT, (selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) => {
            if (otherCollider.node.getComponent(MTRNX_Water_Unit) && this.IsEnemy == otherCollider.node.getComponent(MTRNX_Water_Unit).IsEnemy
                && otherCollider.node.getComponent(MTRNX_Water_Unit).State != 2) {//检测单位是不是友方
                otherCollider.node.getComponent(MTRNX_Water_Unit).Buff_recover(10, 999);
                if (!otherCollider.node.getChildByName("回血特效")) {
                    BundleManager.GetBundle("56_MTRNX_Water_Bundle").load("Prefabs/Effects/回血特效", Prefab, (err, event) => {
                        if (err) {
