...
import { MTRNX_Water_WBullet } from '../MTRNX_Water_WBullet';
import { MTRNX_Water_GameManager } from '../MTRNX_Water_GameManager';

const { ccclass, property } = _decorator;

@ccclass('MTRNX_Water_FangJianKonZhanChe')
export class MTRNX_Water_FangJianKonZhanChe extends MTRNX_Water_Unit {
    @property(Prefab)
    Bullet: Prefab = null;
    ...
    Biu() {
        ...
        this.scheduleOnce(() => {
            ...
            if (this.IsBeginAttack && this.TargetNodes.length > 0) {
                MTRNX_Water_AudioManager.AudioClipPlay("蓝色子弹发射", 0.3);
                let pre: Node = MTRNX_Water_PoolManager.Instance.GetNode(this.Bullet, MTRNX_Water_GameManager.Instance.GameNode.getChildByName("子弹层"));
                pre.getComponent(MTRNX_Water_WBullet).ispenetrate = true;
                var dir = this.forward;
                let linearVelocity = v2(this.BulletSpeed * dir.x, this.BulletSpeed * dir.y);
                pre.getComponent(MTRNX_Water_WBullet).init(this.TargetNodes[0], this.attack, this.BulletSpeed, this.IsEnemy, true, linearVelocity);
