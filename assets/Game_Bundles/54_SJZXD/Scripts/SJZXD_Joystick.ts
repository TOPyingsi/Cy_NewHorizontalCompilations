...
@ccclass('SJZXD_Joystick')
export class SJZXD_Joystick extends Component {
    ...
    OnTouchMove(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                ...
                //     degree += 360;
                // }
                EventManager.Scene.emit(SJZXD_EventManager.MOVEMENT, v3(dirX, dirY, len / radius));
            }
        }
    }
    ...
    OnTouchEnd(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            let touch = touches[i];
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                EventManager.Scene.emit(SJZXD_EventManager.MOVEMENT_STOP);
    ...
    updateDirection() {
        ...
        EventManager.Scene.emit(SJZXD_EventManager.MOVEMENT, v3(this._dir.x, this._dir.y, 1.0));
    }
    //#endregion
}