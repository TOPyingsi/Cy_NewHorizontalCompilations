...
@ccclass('SJZXD_PoolManager')
export class SJZXD_PoolManager extends Component {
    @property({ type: [Prefab] })
    Prefabs: Prefab[] = [];
    private static _instance: SJZXD_PoolManager;

    private _pools: Map<string, NodePool> = new Map(); // 对象池存储
    private _prefabs: Map<string, Prefab> = new Map(); // 预制体资源存储

    public static get Instance(): SJZXD_PoolManager {
        if (!this._instance) {
            this._instance = new SJZXD_PoolManager();
        }
        return this._instance;
    }
    protected onLoad(): void {
        SJZXD_PoolManager._instance = this;
    }
