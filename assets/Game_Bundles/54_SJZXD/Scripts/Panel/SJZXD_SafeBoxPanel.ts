...
@ccclass('SJZXD_SafeBoxPanel')
export class SJZXD_SafeBoxPanel extends PanelBase {
    Panel: Node = null;
    SafeBox: Node = null;
    ...
    OnButtonClick(event: Event) {
        ...
        switch (event.target.name) {
            case "Button":
                let count = 0;
                const wrongFlink = () => {
                    SJZXD_AudioManager.globalAudioPlay("SafeBoxF");
                    this.scheduleOnce(() => {
                        // let color: Color[] = [];
                ...
                const sucessFlink = () => {
                    SJZXD_AudioManager.globalAudioPlay("SafeBoxT");
                    this.scheduleOnce(() => {
                        for (let i = 0; i < this.SafeBox.children.length; i++) {
                        ...
                        this.scheduleOnce(() => {
                            ...
                            count++;
                            if (count >= 5) {
                                SJZXD_UIManager.Instance.HidePanel(SJZXD_Constant.Panel.SafeBoxPanel);
                ...
                break;
            case "Mask": this.Close(); SJZXD_AudioManager.globalAudioPlay("点击");
                break;
        }
