...
@ccclass('SJZXD_Gun')
export class SJZXD_Gun extends SJZXD_I_weapon {
    @property(Prefab)
    ...
    protected onLoad(): void {
        this._WeaponData = SJZXD_Constant.getWeaponDataByName(this.node.name);
    ...
    private async createBullet(): Promise<void> {
        ...
        try {
            SJZXD_AudioManager.globalAudioPlay(this._WeaponData.开枪音效);
            // 计算子弹朝向
            const direction = this.node.forward;
            // 将子弹添加到场景中
            if (this.node.parent.parent.parent) {
                // 读取子弹
                const bulletNode = SJZXD_PoolManager.Instance.Get(this.BulletPre, this.node.parent.parent.parent
                    , this._muzzleNode.worldPosition.clone()
                );
                // 设置子弹的初始速度和方向
                const bulletScript = bulletNode.getComponent(SJZXD_Bullet); // 假设子弹有SJZXD_Bullet组件
                if (bulletScript) {
    ...
    Reload() {
        ...
        SJZXD_AudioManager.globalAudioPlay("换弹音效");
        ...
        if (this.node.parent.parent.name == "Player") {
            director.getScene().emit(SJZXD_EventManager.主角换弹, this._bullettime);
        }
    };
    // 检查是否需要换弹
