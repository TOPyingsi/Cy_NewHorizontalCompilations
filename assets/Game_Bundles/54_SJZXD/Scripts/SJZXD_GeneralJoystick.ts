...
@ccclass('SJZXD_GeneralJoystick')
export class SJZXD_GeneralJoystick extends Component {
    @property()
    Name: string = "";//摇杆唯一标识符
    ...
    OnTouchMove(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                ...
                EventManager.Scene.emit(SJZXD_EventManager.通用摇杆移动未归一化, this.Name, v3(ox, oy, 0));
                ...
                EventManager.Scene.emit(SJZXD_EventManager.通用摇杆移动, this.Name, v3(dirX, dirY, len / radius));
            }
        }
    }
    ...
    OnTouchEnd(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            let touch = touches[i];
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                EventManager.Scene.emit(SJZXD_EventManager.通用摇杆停止, this.Name);
