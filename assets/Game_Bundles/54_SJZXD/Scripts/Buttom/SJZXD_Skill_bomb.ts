...
import { SJZXD_Skill_WuDiHuoPao } from '../Skill/SJZXD_Skill_WuDiHuoPao';
...
export class SJZXD_Skill_bomb extends SJZXD_I_SkillBtn {//无敌炮火技能
    ...
    public SkillAttack: number = 400;//技能基础伤害
    ...
    ShootBullet() {
        ...
        for (let index = 0; index < this.PosArray.length; index++) {
            this.scheduleOnce(() => {
                let bullet = instantiate(this.SkillPre);
                bullet.setParent(SJZXD_GameManager.Instance.GameNode);
                bullet.setWorldPosition(this.Lineofsight.worldPosition.clone().add(this.PosArray[index]));
                bullet.getComponent(SJZXD_Skill_WuDiHuoPao).Show(this.FindUnit.Camp, this.SkillAttack);
            }, index * 0.2)
        }
    }
