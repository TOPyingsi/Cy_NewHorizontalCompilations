...
@ccclass('SJZXD_Enemy_CombatBoss')
export class SJZXD_Enemy_CombatBoss extends Component {    //近战BOSS
    ...
    onStartContact(selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
        if (otherCollider.node.getComponent(SJZXD_Unit)) {
            if (otherCollider.node.getComponent(SJZXD_Unit)?.Camp != this.UnitData.Camp) {
                SJZXD_AudioManager.globalAudioPlay("近战攻击");
                otherCollider.node.getComponent(SJZXD_Unit).TakeDamage(this.Attack);
            }
        }
    }
    //死亡事件
