...
export class XGDY_FishLine extends Component {
    ...
    setSkin(fishingRodId:string){
        XGDY_LoadManager.Instance.getFishingRodIconById(fishingRodId, (spriteFrame:SpriteFrame) => {
            if (spriteFrame) {
    ...
    setStartAndEndNode(startNode:Node,endNode:Node,direction?:number) {
        this.startNode = startNode;
    ...
    updateLine(){
        if(!this.startUpdateLine || !XGDY_DataManager.Instance.dynamicData.isFishingLineOpen)return;
