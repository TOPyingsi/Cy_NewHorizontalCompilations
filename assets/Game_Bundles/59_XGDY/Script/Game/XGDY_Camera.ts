 ...
 */
@ccclass('XGDY_Camera')
export class XGDY_Camera extends Component {
     ...
     */
      public followPlayer(dt:number)
      {
        if(!this.followTarget ||!this.followTarget.worldPosition)return;
          ...
          const viewport = view.getVisibleSize();
          let cameraRightLimt = XGDY_DataManager.Instance.dynamicData.mapWidth/2 - viewport.width/2 + viewport.width/2;
          let cameraLeftLimt = -XGDY_DataManager.Instance.dynamicData.mapWidth/2 + viewport.width/2 + viewport.width/2;
          let cameraTopLimt = XGDY_DataManager.Instance.dynamicData.mapHeight/2 - viewport.height/2 + viewport.height/2;
          let cameraBottomLimt = -XGDY_DataManager.Instance.dynamicData.mapHeight/2 + viewport.height/2  + viewport.height/2;
    ...
    //   }
      ...
      updateNewTarget(speedScale = 2,isOrginHight = true){
        this.setFollowTarget(XGDY_DataManager.Instance.dynamicData.cameraTarget,speedScale,isOrginHight);
      ...
      addEventListener(){
        EventManager.on(XGDY_GameEvents.Update_Camera_Tartget,this.updateNewTarget,this)
      }

      removeListener(){
        EventManager.off(XGDY_GameEvents.Update_Camera_Tartget,this.updateNewTarget,this)
