...
@ccclass('XGDY_AudioManager')
export class XGDY_AudioManager extends Component {
    private static instance: XGDY_AudioManager;
    ...
    // 单例模式
    public static getInstance(): XGDY_AudioManager {
        if (!XGDY_AudioManager.instance) {
            // 查找场景中是否已存在GameManager节点
            const existingNode = director.getScene().getChildByName('GameManager');
            if (existingNode) {
                XGDY_AudioManager.instance = existingNode.getComponent(XGDY_AudioManager);
            } else {
                // 如果不存在，则创建新节点并添加组件
                const gameManagerNode = new Node('GameManager');
                director.getScene().addChild(gameManagerNode);
                XGDY_AudioManager.instance = gameManagerNode.addComponent(XGDY_AudioManager);
            }
        }
        return XGDY_AudioManager.instance;
    }

    protected onLoad(): void {
        XGDY_AudioManager.instance = this;
