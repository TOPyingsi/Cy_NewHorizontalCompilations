 ...
 */
@ccclass('XGDY_AiSkillManager')
export class XGDY_AiSkillManager extends Component {
public static Instance: XGDY_AiSkillManager;  // 单例实例，全局访问点

  private skillAlgorithms: XGDY_SkillUpdateItem[] = []; // 生效中的伤害算法列表


    onLoad() {
        XGDY_AiSkillManager.Instance = this;
        this.addListener();
    }
    ...
    stopPullLine(){
        ...
        XGDY_AiSkillManager.Instance.removeSkill(XGDY_SkillId.正常扣血)
        this.pullLine(0);
    }
  ...
  private pullLine(pull: number) {
    XGDY_DataManager.Instance.dynamicData.reversePullForce = pull;
  }
  ...
  private onFishDie() {
    ...
    if(XGDY_DataManager.Instance.dynamicData.is_Map103_Challenge_1_Challengeing && XGDY_DataManager.Instance.dynamicData.Map103_challenge_1_Count < (XGDY_DataManager.Instance.dynamicData.Map103_Challenge_1_TargetFishCount-1)){
