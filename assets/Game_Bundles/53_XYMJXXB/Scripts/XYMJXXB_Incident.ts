...
@ccclass('XYMJXXB_Incident')
export class XYMJXXB_Incident extends Component {
    public static LoadSprite(Path: string) {
        return new Promise((resolve, reject) => {
            BundleManager.GetBundle("53_XYMJXXB").load(Path + "/spriteFrame", SpriteFrame, (err, data) => {
                if (err) {
    ...
    public static Loadprefab(Path: string) {
        return new Promise((resolve, reject) => {
            BundleManager.GetBundle("53_XYMJXXB").load(Path, Prefab, (err, data) => {
                if (err) {
