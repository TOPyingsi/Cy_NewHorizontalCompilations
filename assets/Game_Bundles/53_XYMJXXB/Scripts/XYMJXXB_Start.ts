...
import { XYMJXXB_lotteryPanel } from './Panel/XYMJXXB_lotteryPanel';
import { XYMJXXB_conversionPanel } from './Panel/XYMJXXB_conversionPanel';
const { ccclass, property } = _decorator;

@ccclass('XYMJXXB_Start')
export class XYMJXXB_Start extends Component {
    @property(Node)
    ...
    OnbuttomClick(btn: EventTouch) {
        ...
        switch (btn.target.name) {
            ...
            case "学位升级":
                ...
                break;
            case "皮肤抽奖1":
                XYMJXXB_lotteryPanel.lotteryPanelIndex = 0;
                this.Canvas.getChildByName("抽奖界面").active = true;
                ProjectEventManager.emit(ProjectEvent.弹出窗口);
                break;
            case "皮肤抽奖2":
                XYMJXXB_lotteryPanel.lotteryPanelIndex = 1;
                this.Canvas.getChildByName("抽奖界面").active = true;
                ProjectEventManager.emit(ProjectEvent.弹出窗口);
                break;
            case "战令":
                ...
                break;
            case "兑换1":
                XYMJXXB_conversionPanel.conversionPanelIndex = 0;
                this.Canvas.getChildByName("兑换界面").active = true;
                ProjectEventManager.emit(ProjectEvent.弹出窗口);
                break;
            case "兑换2":
                XYMJXXB_conversionPanel.conversionPanelIndex = 1;
                this.Canvas.getChildByName("兑换界面").active = true;
                ProjectEventManager.emit(ProjectEvent.弹出窗口);
                break;
            case "兑换3":
                XYMJXXB_conversionPanel.conversionPanelIndex = 2;
                this.Canvas.getChildByName("兑换界面").active = true;
                ProjectEventManager.emit(ProjectEvent.弹出窗口);
                break;
            case "兑换4":
                XYMJXXB_conversionPanel.conversionPanelIndex = 3;
                this.Canvas.getChildByName("兑换界面").active = true;
                ProjectEventManager.emit(ProjectEvent.弹出窗口);
                break;
            case "挂机模式":
            ...
            case "返回":
                ProjectEventManager.emit(ProjectEvent.返回主页按钮事件, () => {
                    UIManager.ShowPanel(Panel.LoadingPanel, GameManager.StartScene, () => {
                        ProjectEventManager.emit(ProjectEvent.返回主页, "校园摸金修仙版");
                    });
                });
