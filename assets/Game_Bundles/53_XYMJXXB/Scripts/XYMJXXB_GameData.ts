...
@ccclass('XYMJXXB_GameData')
export class XYMJXXB_GameData extends Component {
    private static _instance: XYMJXXB_GameData = null;
    public static get Instance(): XYMJXXB_GameData {
        if (!this._instance) {
            this.ReadDate();
        }
    ...
    public Skin: string = "小龙人";//当前选择皮肤
    public SkinData: string[] = ["小龙人", "敖丙", "妖狐", "麒麟"];//已经解锁的皮肤
    ...
    public AddHP: number = 0;//额外生命
    public AddATKSpeed: number = 0;//额外攻速
    public AddMoveSpeed: number = 0;//额外移速
    ...
    public static DateSave() {
        let json = JSON.stringify(XYMJXXB_GameData.Instance);
        sys.localStorage.setItem("XYMJXXB_DATA", json);
        console.log("游戏存档");
    }
    public static ReadDate() {
        let name = sys.localStorage.getItem("XYMJXXB_DATA");
        if (name != "" && name != null) {
            console.log("读取存档");
            XYMJXXB_GameData._instance = Object.assign(new XYMJXXB_GameData(), JSON.parse(name));
        } else {
            console.log("新建存档");
            XYMJXXB_GameData._instance = new XYMJXXB_GameData();
