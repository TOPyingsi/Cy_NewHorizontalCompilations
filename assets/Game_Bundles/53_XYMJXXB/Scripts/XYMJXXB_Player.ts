...
import { XYMJXXB_Pile } from './XYMJXXB_Pile';
...
import { XYMJXXB_AIPlayer } from './XYMJXXB_AIPlayer';
import { XYMJXXB_Bullet } from './XYMJXXB_Bullet';
import { XYMJXXB_AudioManager } from './XYMJXXB_AudioManager';
import { ProjectEvent, ProjectEventManager } from 'db://assets/Scripts/Framework/Managers/ProjectEventManager';
import { XYMJXXB_Lock } from './XYMJXXB_Lock';
import { UIManager } from 'db://assets/Scripts/Framework/Managers/UIManager';
import { XYMJXXB_BagProp } from './XYMJXXB_BagProp';
const { ccclass, property } = _decorator;

@ccclass('XYMJXXB_Player')
export class XYMJXXB_Player extends Component {
    ...
    //加成计算
    Addcalculate() {
        this.maxHealth = 500 + XYMJXXB_GameData.Instance.AddHP;
        this.moveSpeed = 3 + XYMJXXB_GameData.Instance.AddMoveSpeed;
    ...
    update(deltaTime: number) {
        if (XYMJXXB_GameManager.Instance.isGameOver) {
            return;
        }
        if (!this.isInit) {
            return;
    ...
    curPlie: XYMJXXB_Pile = null;
    AIArray: XYMJXXB_AIPlayer[] = [];
    onStartContact(selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
        ...
        if (group.name === "AIPlayer") {
            ...
            let AITs = group.getComponent(XYMJXXB_AIPlayer);
        ...
        if (group.name === "横锁" || group.name === "竖锁") {
            ...
            let lockTs = otherCollider.node.getComponent(XYMJXXB_Lock);
            let isHasKey = XYMJXXB_GameData.Instance.GetPropNum(lockTs.keyName);
            //碰到锁
        ...
        if (group.name === "BagProp") {
            ...
            this.PickUpBtn.active = true;
            this.propNameSelf = group.getComponent(XYMJXXB_BagProp).propName;
            return;
        }

        let isPlie = group.name.startsWith("Plie");
        if (isPlie) {
            this.curPlie = otherCollider.node.getComponent(XYMJXXB_Pile);
        ...
        if (group.name === "撤离点") {
            XYMJXXB_GameManager.Instance.startExtrac();
        }
    }
    ...
    onExitContact(selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
        ...
        if (group === "AIPlayer") {
            if (otherCollider.node.getComponent(XYMJXXB_AIPlayer)) {
                if (this.AIArray.indexOf(otherCollider.node.getComponent(XYMJXXB_AIPlayer)) != -1) {
                    this.AIArray.splice(this.AIArray.indexOf(otherCollider.node.getComponent(XYMJXXB_AIPlayer)), 1);
        ...
        if (group === "撤离点") {
            XYMJXXB_GameManager.Instance.exitExtrac();
        }
    }
    ...
    onMove(Dir: Vec3) {
        ...
        XYMJXXB_Player.couldMove = true;
    }

    onStopMove() {

        XYMJXXB_Player.couldMove = false;
        this._dir = v3(0, 0, 0);

    }

    curLockTs: XYMJXXB_Lock = null;
    PickUp() {
        ...
        //     XYMJXXB_GameManager.Instance.gameBagTs.addPropToBagByName(this.propNameSelf);
        // }
        ...
        XYMJXXB_AudioManager.globalAudioPlay("捡东西");
    ...
    Attack(Dir: Vec3) {
        ...
        this.handPropData = XYMJXXB_Constant.GetDataByName(this.handProp.name);

        if (this.handPropData.type !== "武器" || !this.handPropData) {
            return;
        }

        this.handPropData = XYMJXXB_Constant.GetWeaponDataByName(this.handProp.name);

        let attack = this.handPropData.Attack;
        let attackSpeed = this.handPropData.AttackSpeed;

        // XYMJXXB_Player.couldAttack = true;
        this.isAttacking = true;


        let attackInterval = 1 / (attackSpeed * (1 + XYMJXXB_GameData.Instance.AddATKSpeed));
        ...
        if (propName === "极简武力" || propName === "怒火手枪" || propName === "冰霜神话" || propName === "冲锋枪"
            ...
            XYMJXXB_AudioManager.globalAudioPlay("枪声");

            let bulletNode = instantiate(this.bulletPrefab);

            let bulletTs = bulletNode.getComponent(XYMJXXB_Bullet);
            bulletTs.ATK = attack;
        ...
        XYMJXXB_AudioManager.globalAudioPlay("近战击中");
    ...
    onAttack(damage: number) {
        ...
        if (this.curHealth <= 0) {
            this.curHealth = 0;
            XYMJXXB_Player.couldAttack = false;
            this.isAttacking = false;
            this.node.active = false;
            XYMJXXB_GameData.Instance.KnapsackData = [];
            XYMJXXB_GameManager.Instance.lost();
            // director.loadScene("XYMJXXB_Start");
    ...
    initData() {
        ...
        XYMJXXB_Incident.LoadSprite("Sprites/皮肤/" + XYMJXXB_GameData.Instance.Skin).then((sp: SpriteFrame) => {
            if (this.node?.isValid) {
                this.node.getChildByName("角色图").getComponent(Sprite).spriteFrame = sp;
            }
        });

        XYMJXXB_Incident.Loadprefab("Prefabs/子弹").then((prefab: Prefab) => {
            if (this.node?.isValid) {
        ...
        XYMJXXB_GameManager.Instance.player = this;
    }
