...
import { XYMJXXB_Prop } from './XYMJXXB_Prop';
...
//开堆
@ccclass('XYMJXXB_Pile')
export class XYMJXXB_Pile extends Component {
    ...
    creatProp() {
        ...
        for (let i = 0; i < this.pileMaxContain; i++) {
            ...
            if (i === 0) {
                // 获取丹药池
                const pillPool = XYMJXXB_Constant.GetPillData();
                // 从丹药池中随机抽取一个
                propData = pillPool[Math.floor(Math.random() * pillPool.length)];
            } else {
                //随机奖池品质
                let randomQuality: number = math.randomRange(this.minPercent, 1) * this.qualityPool.length;
                let propQuality: string = XYMJXXB_Constant.QualityType[Math.floor(randomQuality)];
                //随机抽一个稀有度池子
                let qualityPool = XYMJXXB_GameManager.Instance.propPoolMap.get(propQuality);
                //从池子中随机抽一个道具
                propData = qualityPool[Math.floor(Math.random() * qualityPool.length)];
            }
            ...
            prop.parent = Pile;
            let propTs = prop.getComponent(XYMJXXB_Prop);
            ...
            console.log("第" + (i + 1) + "个道具为" + propData.Name + "，品质为" + propData.quality + "，价值为" + propData.value);
        }
    ...
    getProp(index: number) {
        ...
        if (XYMJXXB_GameData.Instance.pushKnapsackData(this.pilePropTsArr[index].propName, 1)) {
            ...
            this.getNum++;
            XYMJXXB_GameData.Instance.GameData[1] += 1;
    ...
    getPool() {
        switch (XYMJXXB_Constant.level) {
            case 1:
                this.qualityPool = Array.from(XYMJXXB_Constant.QualityType);
