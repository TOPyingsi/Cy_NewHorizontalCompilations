...
@ccclass('XYMJXXB_AIPlayer')
export class XYMJXXB_AIPlayer extends Component {
    ...
    start() {
        this.maxHealth = XYMJXXB_Constant.level * XYMJXXB_Constant.level * 50 + 200;
        this.curHealth = XYMJXXB_Constant.level * XYMJXXB_Constant.level * 50 + 200;
        this.ATK = XYMJXXB_Constant.level * 10 + 20;
        ...
        let SkinName: string = XYMJXXB_AIPlayer.SkinData[Math.floor(Math.random() * XYMJXXB_AIPlayer.SkinData.length)];
        XYMJXXB_Incident.LoadSprite("Sprites/皮肤/" + SkinName).then((sp: SpriteFrame) => {
    ...
    Attack(Dir: Vec3) {
        ...
        let handPropData = XYMJXXB_Constant.GetWeaponDataByName(this.handProp.name);
        ...
        this.scheduleOnce(() => {
            ...
            this.playerNode.getComponent(XYMJXXB_Player).onAttack(attck);
    ...
    onAttack(damage: number) {
        ...
        if (this.curHealth <= 0 && this.isDie == false) {
            XYMJXXB_GameData.Instance.GameData[1] += 5;
            XYMJXXB_AudioManager.globalAudioPlay("击杀");
            this.curHealth = 0;
            this.isDie = true;
            this.DieAni();
        }
        ...
        if (XYMJXXB_GameManager.Instance.player) {
            let distance = this.node.worldPosition.clone().subtract(XYMJXXB_GameManager.Instance.player.node.worldPosition.clone());
            let dir = distance.clone().normalize().negative();
            tween(this.node)
                .to(0.2, { position: this.node.position.clone().add(dir.multiplyScalar(-15)) }, { easing: "backOut" })
                .start();
            XYMJXXB_AudioManager.globalAudioPlay("受击");
        }
