...
import { XYMJXXB_Player } from './XYMJXXB_Player';
import { ProjectEvent, ProjectEventManager } from 'db://assets/Scripts/Framework/Managers/ProjectEventManager';
import { XYMJXXB_Incident } from './XYMJXXB_Incident';
import { XYMJXXB_GameData } from './XYMJXXB_GameData';
import { XYMJXXB_GameBag } from './XYMJXXB_GameBag';
const { ccclass, property } = _decorator;

@ccclass('XYMJXXB_GameManager')
export class XYMJXXB_GameManager extends Component {
    ...
    public player: XYMJXXB_Player = null;
    ...
    @property(XYMJXXB_GameBag)
    public gameBagTs: XYMJXXB_GameBag = null;

    public static Instance: XYMJXXB_GameManager = null;

    // public static get Instance(): XYMJXXB_GameManager {
    //     if (!XYMJXXB_GameManager.instance) {
    //         XYMJXXB_GameManager.instance = new XYMJXXB_GameManager();
    //     }
    //     return XYMJXXB_GameManager.instance;
    // }

    start() {
        // PhysicsSystem2D.instance.debugDrawFlags = 1;
        XYMJXXB_GameManager.Instance = this;
        this.initPropData();
    ...
    updateValue() {
        ...
        for (let i = 0; i < XYMJXXB_GameData.Instance.KnapsackData.length; i++) {
            ...
            let propData = XYMJXXB_Constant.GetDataByName(XYMJXXB_GameData.Instance.KnapsackData[i].Name);
            let propValue = propData.value;
            let propNum = XYMJXXB_GameData.Instance.KnapsackData[i].Num;
        ...
        if (this.isFirst) {
            ...
            let valueStr = "当前物品价值：" + XYMJXXB_Incident.GetMaxNum(this.totalValue);
            this.valueLabel.string = valueStr;
            return;
        }

        this.newValue = this.totalValue - this.startValue;
        let valueStr = "当前物品价值：" + XYMJXXB_Incident.GetMaxNum(this.newValue);
        this.valueLabel.string = valueStr;
    ...
    win() {
        ...
        XYMJXXB_GameData.Instance.GameData[1] += 50;
        ...
        tween(this.winNode)
            ...
            .call(() => {
                ...
                value.active = true;
                value.getComponent(Label).string = "撤离物品价值：" + XYMJXXB_Incident.GetMaxNum(this.newValue);
    ...
    //初始化道具数据
    initPropData() {
        for (let j = 0; j < XYMJXXB_Constant.QualityType.length; j++) {
            this.propPoolMap.set(XYMJXXB_Constant.QualityType[j], []);
        }

        for (let i = 0; i < XYMJXXB_Constant.PropData.length; i++) {
            let quality = XYMJXXB_Constant.PropData[i].quality;
            let data = XYMJXXB_Constant.PropData[i];
            if (data.type === "回收物") {
        ...
        XYMJXXB_Incident.Loadprefab("Prefabs/场景/" + XYMJXXB_Constant.mapID).then((prefab) => {
            ...
            let playerTs = curMap.getChildByName("Player").getComponent(XYMJXXB_Player);
            playerTs.initData();
        });
