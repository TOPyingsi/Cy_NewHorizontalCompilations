import { _decorator, Component, Node, Vec2, Vec3, find, instantiate, Prefab, Material, Sprite, tween, v3, Tween, director, sys, v2 } from 'cc';
const { ccclass, property } = _decorator;

@ccclass('GML_ZTool')
export class GML_ZTool extends Component {
    ...
    public static Shaking(node: Node, gap: number, randomMax: number, stopTime: number, callback: Function = null) {
        ...
        tween(node)
            .to(gap, { worldPosition: v3(x + GML_ZTool.GetRandom(-randomMax, randomMax), y + GML_ZTool.GetRandom(-randomMax, randomMax), z) })
            .to(gap, { worldPosition: v3(x + GML_ZTool.GetRandom(-randomMax, randomMax), y + GML_ZTool.GetRandom(-randomMax, randomMax), z) })
            .to(gap, { worldPosition: v3(x + GML_ZTool.GetRandom(-randomMax, randomMax), y + GML_ZTool.GetRandom(-randomMax, randomMax), z) })
            .to(gap, { worldPosition: v3(x + GML_ZTool.GetRandom(-randomMax, randomMax), y + GML_ZTool.GetRandom(-randomMax, randomMax), z) })
            .to(gap, { worldPosition: v3(x + GML_ZTool.GetRandom(-randomMax, randomMax), y + GML_ZTool.GetRandom(-randomMax, randomMax), z) })
   ...
   */
    ...
    */
    public static checkIsLowPhone() {
        ...
        return Number(GML_ZTool.getBenchmarkLevel()) < checkBenchmark;
    }
    ...
    public static RandomPointAroundRange(point: Vec2, radius: number, startPI: number, endPI: number): { x: number, y: number } {
        const radian = Math.random() * (Math.PI * (endPI - startPI)) + Math.PI * startPI;
        return v2(point.x + radius * Math.cos(radian), point.y + radius * Math.sin(radian));
    }
}