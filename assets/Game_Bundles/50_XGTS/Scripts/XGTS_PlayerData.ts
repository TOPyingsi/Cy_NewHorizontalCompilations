...
import { XGTS_ItemData, XGTS_ItemType, XGTS_WorkbenchType } from "./XGTS_Data";
...
export class XGTS_PlayerData {
    constructor(private money: number = 1000000, public InventoryItemData: XGTS_ItemData[] = [], public Weapon_Primary: XGTS_ItemData = null, public Weapon_Secondary: XGTS_ItemData = null, public Weapon_Pistol: XGTS_ItemData = null, public Weapon_Melee: XGTS_ItemData = null, public Weapon_Helmet: XGTS_ItemData = null, public Weapon_BodyArmor: XGTS_ItemData = null,
        public PocketData: XGTS_ItemData[][] = [], public ChestRigData: XGTS_ItemData = null, public BackpackData: XGTS_ItemData = null, private safeBox = null,
        public ShowcaseData: XGTS_ItemData[] = []) {
        if (safeBox) this.SafeBox = safeBox;
        this.Money = money;

        this.InventoryGridCtrl = new XGTS_InventoryGrid(1, 1);

        //口袋
        this.SingleGrids = [];
        for (let i = 0; i < 5; i++) {
            this.SingleGrids.push(new XGTS_InventoryGrid(1, 1));
        }

        if (this.ChestRigData) {
            this.ChestRigGrid = new XGTS_InventoryGrid(this.ChestRigData.EquipData.CarryingSpace.width, this.ChestRigData.EquipData.CarryingSpace.height);
        }

        if (this.BackpackData) {
            this.BackpackGrid = new XGTS_InventoryGrid(this.BackpackData.EquipData.CarryingSpace.width, this.BackpackData.EquipData.CarryingSpace.height);
        }

        //保险箱
        this.SafeBoxGrid = new XGTS_InventoryGrid(this.SafeBox.width, this.SafeBox.height);
    ...
    public set Money(value: number) {
        ...
        this.money = value;
        EventManager.Scene.emit(XGTS_Constant.Event.REFRESH_MONEY);
    }

    /**安全箱 */
    SafeBox: { width: number, height: number, ItemData: XGTS_ItemData[] } = { width: 2, height: 1, ItemData: [] };

    SingleGrids: XGTS_InventoryGrid[] = [];
    ChestRigGrid: XGTS_InventoryGrid = null;
    BackpackGrid: XGTS_InventoryGrid = null;
    SafeBoxGrid: XGTS_InventoryGrid = null;

    //#region 仓库

    InventoryGridCtrl: XGTS_InventoryGrid = null;

    private Init(width: number, height: number) {
        this.InventoryGridCtrl = new XGTS_InventoryGrid(width, height);
        let data = this.InventoryItemData;

        for (let i = 0; i < data.length; i++) {
            //数据
            XGTS_InventoryGrid.PlaceItem(this.InventoryGridCtrl, data[i].Point.x, data[i].Point.y, data[i].Size.width, data[i].Size.height);
        }
    }

    EquipChestRig(chestRig: XGTS_ItemData) {
        if (chestRig.Type != XGTS_ItemType.ChestRig) {
    ...
    EquipBackpack(backpack: XGTS_ItemData) {
        if (backpack.Type != XGTS_ItemType.Backpack) {
    ...
    SetSafeBox(width: number, height: number) {
        XGTS_DataManager.PlayerData.SafeBox = { width: width, height: height, ItemData: [] };
        this.SafeBoxGrid = new XGTS_InventoryGrid(width, height);
    }

    AddItemToBackpack(data: XGTS_ItemData) {
        if (this.BackpackData && this.BackpackData.EquipData) {
            XGTS_DataManager.FillContainer(this.BackpackGrid, [data], this.BackpackData.EquipData.ItemData);
        }
    }

    /**获取工作台等级 */
    GetWorkbenchLv(type: XGTS_WorkbenchType): number {
        return PrefsManager.GetNumber(`WorkbenchLv_${type}`, 0);
    }

    /**设置工作台等级 */
    SetWorkbenchLv(type: XGTS_WorkbenchType, value: number) {
        value = misc.clampf(value, 1, 3);
        PrefsManager.SetNumber(`WorkbenchLv_${type}`, value);
    }

    /**添加物品到仓库 */
    AddItemToInventory(data: XGTS_ItemData) {
        ...
        for (let j = 0; j < gridLength; j++) {
            ...
            if (this.InventoryGridCtrl.grid[y][x] == 0) {
                if (XGTS_InventoryGrid.CanPlaceItem(this.InventoryGridCtrl, x, y, data.Size.width, data.Size.height)) {
                    XGTS_InventoryGrid.PlaceItem(this.InventoryGridCtrl, x, y, data.Size.width, data.Size.height);
                    data.Point.x = x;
                    data.Point.y = y;
    ...
    /**从仓库移除物品 */
    RemoveItemFromInventory(data: XGTS_ItemData) {
        if (this.InventoryItemData.indexOf(data) != -1) {
            this.InventoryItemData.splice(this.InventoryItemData.indexOf(data), 1);
            XGTS_DataManager.SaveData();
            XGTS_InventoryGrid.RemoveItem(this.InventoryGridCtrl, data.Point.x, data.Point.y, data.Size.width, data.Size.height);
    ...
    RemoveItemFromBackpack(data: XGTS_ItemData) {
        ...
        for (let i = 0; i < this.PocketData.length; i++) {
            ...
            if (pocket.findIndex(e => e == data) !== -1) {
                ...
                XGTS_InventoryGrid.RemoveItem(this.SingleGrids[i], data.Point.x, data.Point.y, data.Size.width, data.Size.height);
            }
        }
        ...
        if (this.ChestRigData) {
            if (this.ChestRigData.EquipData.ItemData.findIndex(e => e == data) !== -1) {
                ...
                XGTS_InventoryGrid.RemoveItem(this.ChestRigGrid, data.Point.x, data.Point.y, data.Size.width, data.Size.height);
            }
        }
        ...
        if (this.SafeBox) {
            if (this.SafeBox.ItemData.findIndex(e => e == data) !== -1) {
                ...
                XGTS_InventoryGrid.RemoveItem(this.SafeBoxGrid, data.Point.x, data.Point.y, data.Size.width, data.Size.height);
            }
        }

        EventManager.Scene.emit(XGTS_Constant.Event.REFRESH_BACKPACK_ITEMS);
    }
    ...
    ClearBackpack() {
        ...
        for (let i = 0; i < this.PocketData.length; i++) {
            const pocket = this.PocketData[i];
            pocket.forEach(e => {
                XGTS_InventoryGrid.RemoveItem(this.SingleGrids[i], e.Point.x, e.Point.y, e.Size.width, e.Size.height);
            });
            pocket.length = 0;
        }
    ...
    /**获得身上所有的子弹 */
    GetBackpackAllItemByType(type: XGTS_ItemType) {
        let allData: XGTS_ItemData[] = [];
    ...
    GetAmmoByType(ammoType: number, needCount: number) {
        ...
        for (let i = 0; i < this.PocketData.length; i++) {
            const pocket = this.PocketData[i];
            const data = pocket.find(e => e.Type == XGTS_ItemType.Ammo && e.AmmoData.Type == ammoType);
            if (data) {
        ...
        //胸挂
        if (this.ChestRigData) {
            const data = this.ChestRigData.EquipData.ItemData.find(e => e.Type == XGTS_ItemType.Ammo && e.AmmoData.Type == ammoType);
            if (data) {
                if (needCount >= data.Count) {
                    Tools.RemoveItemFromArray(this.ChestRigData.EquipData.ItemData, data);
                    console.error("移除", this.ChestRigData.EquipData.ItemData, data);
                }
                return data;
            }
        }
        ...
        //背包
        if (this.BackpackData) {
            const data = this.BackpackData.EquipData.ItemData.find(e => e.Type == XGTS_ItemType.Ammo && e.AmmoData.Type == ammoType);
            if (data) {
        ...
        //保险箱
        if (this.SafeBox) {
            const data = this.SafeBox.ItemData.find(e => e.Type == XGTS_ItemType.Ammo && e.AmmoData.Type == ammoType);
            if (data) {
    ...
    GetFirstAmmoType() {
        ...
        for (let i = 0; i < this.PocketData.length; i++) {
            const pocket = this.PocketData[i];
            const data = pocket.find(e => e.Type == XGTS_ItemType.Ammo);
            if (data) {
                // if (needCount >= data.Count) {
                //     Tools.RemoveItemFromArray(pocket, data);
                // }
                return data.AmmoData.Type;
            }
        }

        //胸挂
        if (this.ChestRigData) {
            const data = this.ChestRigData.EquipData.ItemData.find(e => e.Type == XGTS_ItemType.Ammo);
            if (data) {
                return data.AmmoData.Type;
            }
        }
        ...
        //背包
        if (this.BackpackData) {
            const data = this.BackpackData.EquipData.ItemData.find(e => e.Type == XGTS_ItemType.Ammo);
            if (data) {
                 return data.AmmoData.Type;
            }
        }
        ...
        //保险箱
        if (this.SafeBox) {
            const data = this.SafeBox.ItemData.find(e => e.Type == XGTS_ItemType.Ammo );
            if (data) {
                  return data.AmmoData.Type;
            }
        }
    ...
    GetAmmoCountByType(ammoType: number) {
        ...
        for (let i = 0; i < this.PocketData.length; i++) {
            for (let j = 0; j < this.PocketData[i].length; j++) {
                ...
                if (e.Type == XGTS_ItemType.Ammo && e.AmmoData.Type == ammoType) count += e.Count;
            }
        }
    ...
    /**获得仓库里所有该类型的物品 */
    GetInventoryItemByType(type: XGTS_ItemType) {
