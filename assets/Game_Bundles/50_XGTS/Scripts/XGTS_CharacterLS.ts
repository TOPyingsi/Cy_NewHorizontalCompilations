...
@ccclass('XGTS_CharacterLS')
export default class XGTS_CharacterLS extends XGTS_CharacterController {
    ...
    RefreshEquip() {
        let weaponData = XGTS_DataManager.ItemDataMap.get(XGTS_ItemType.Weapon).find(e => e.Name == "S12K");
        //5.56x45mm M9
        let ammo = XGTS_DataManager.GetItemDataByType(XGTS_ItemType.Ammo, 50851);
        if (!ammo) {
    ...
    InitDeadBox() {
        ...
        node.getComponent(XGTS_Supplies).InitCharacterCase(true);
    }
    ...
    update(dt) {
        ...
        if (XGTS_GameManager.Instance.player.node.getWorldPosition().clone().subtract(this.node.getWorldPosition().clone()).length() > 3000 || this.lostTargetTimer > this.lostTargetTime) {
            this.StopFire();
            this.needMove = false;
        ...
        this.results = PhysicsSystem2D.instance.raycast(this.node.worldPosition, XGTS_GameManager.Instance.player.node.worldPosition, ERaycast2DType.Closest);
    ...
    StartFire() {
        ...
        XGTS_AudioManager.Instance.PlaySFX(XGTS_Audio.Fire, 0.3);
    ...
    onBeginContact(selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
        if (otherCollider.node.name == "Bullet") {
            ...
            if (bullet.target.rigidbody.group == XGTS_Constant.Group.Enemy) return;
