...
@ccclass('XGTS_ItemInfoPanel')
export default class XGTS_ItemInfoPanel extends PanelBase {
    Panel: Node = null;
    Icon: Sprite = null;
    ...
    Show(data: XGTS_ItemData, showButtons: boolean = false, callback: Function = null): void {
        ...
        this.Button_GaiZhuang.active = XGTS_ItemData.IsGun(data.Type);

        this.TitleLabel.string = `${data.Name}`;
        this.MoneyLabel.string = `${data.Price}`;

        if (data.Type == XGTS_ItemType.Weapon) {
            let ammoData = XGTS_DataManager.AmmoData.filter(e => e.Type == data.WeaponData.AmmoType);
            let ammoStr = ``;
        ...
        BundleManager.LoadSpriteFrame(GameManager.GameData.DefaultBundle, `Res/Items/${data.ImageId}`).then((sf: SpriteFrame) => {
            ...
            XGTS_GameManager.SetImagePreferScale(this.Icon, 300, 300);
        });
    ...
    OnButtonClick(event: Event) {
        ...
        switch (event.target.name) {
            case "Mask":
                XGTS_UIManager.Instance.HidePanel(XGTS_Constant.Panel.ItemInfoPanel);
                break;
            case "Button_Sell":
            ...
            case "Button_GaiZhuang":
                BundleManager.LoadPrefab(GameManager.GameData.DefaultBundle, `UI/PlayerInventory`).then((prefab: Prefab) => {
                    ...
                    XGTS_UIManager.Instance.ShowPanel(XGTS_Constant.Panel.AccessoriesPanel, [this.data, spawnInverntory]);
                });
