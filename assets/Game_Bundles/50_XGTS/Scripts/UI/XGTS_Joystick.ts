...
@ccclass('XGTS_Joystick')
export class XGTS_Joystick extends Component {
    ...
    OnTouchMove(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                ...
                EventManager.Scene.emit(XGTS_Constant.Event.MOVEMENT, dirX, dirY, len / radius);
            }
        }
    }
    ...
    OnTouchEnd(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            let touch = touches[i];
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                EventManager.Scene.emit(XGTS_Constant.Event.MOVEMENT_STOP);
    ...
    updateDirection() {
        ...
        EventManager.Scene.emit(XGTS_Constant.Event.MOVEMENT, this._dir.x, this._dir.y, 1.0);
    }
    //#endregion
}