...
@ccclass('XGTS_GamePanel')
export default class XGTS_GamePanel extends Component {
    public static Instance: XGTS_GamePanel = null;
    ...
    RefreshMedItems() {
        ...
        let medData: XGTS_ItemData[] = XGTS_DataManager.PlayerDatas[0].GetBackpackAllItemByType(XGTS_ItemType.MedicalItem);
        medData.push(...XGTS_DataManager.PlayerDatas[0].GetBackpackAllItemByType(XGTS_ItemType.ArmorItem));

        if (medData.length > 0) {
            for (let i = 0; i < medData.length; i++) {
                let node = XGTS_PoolManager.Instance.Get(XGTS_Constant.Prefab.CommonItem, this.MedicineScrollView.getChildByPath("view/MedicineContent"));
    ...
    MedCallback(data: XGTS_ItemData) {
        if (data.Type == XGTS_ItemType.MedicalItem) {
            let player = XGTS_GameManager.Instance.player;
            if (player && player.HP < player.MaxHP) {
                this.ShowUseItem(true, data, (data: XGTS_ItemData) => {
                    ...
                    if (addHP > data.ConsumableData.Durability) {
                        ...
                        XGTS_DataManager.PlayerDatas[0].RemoveItemFromBackpack(data);
                    } else {
        ...
        if (data.Type == XGTS_ItemType.ArmorItem) {
            if (XGTS_DataManager.PlayerDatas[0].Weapon_Helmet != null && data.Name.includes(`头盔`)) {
                this.ShowUseItem(true, data, (data: XGTS_ItemData) => {
                    let equip = XGTS_DataManager.PlayerDatas[0].Weapon_Helmet.EquipData;
            ...
            } else if (XGTS_DataManager.PlayerDatas[0].Weapon_BodyArmor != null && data.Name.includes(`护甲`)) {
                this.ShowUseItem(true, data, (data: XGTS_ItemData) => {
                    let equip = XGTS_DataManager.PlayerDatas[0].Weapon_BodyArmor.EquipData;
                    let addDurability = equip.MaxDurability - equip.Durability;
    ...
    Reload() {
        if (XGTS_GameManager.Instance.player.weapon) {
            ...
            let ammoData = XGTS_DataManager.PlayerDatas[0].GetAmmoByType(weaponData.AmmoType, needCount);
            if (ammoData) {
    ...
    RefreshAmmoItems() {
        ...
        let ammoData: XGTS_ItemData[] = XGTS_DataManager.PlayerDatas[0].GetBackpackAllItemByType(XGTS_ItemType.Ammo);
    ...
    protected update(dt: number): void {
        if (this.startCount) {
            ...
            if (this.countDown <= 0) {
                ...
                XGTS_LvManager.Instance.matchData.Reward = XGTS_DataManager.PlayerDatas[0].GetTotalValue();
        ...
        switch (event.target.name) {
            ...
            case "Weapon_0":
                if (XGTS_DataManager.PlayerDatas[0].Weapon_Primary) {
                    XGTS_GameManager.Instance.player.SetGun(XGTS_DataManager.PlayerDatas[0].Weapon_Primary);
                }
                break;
            case "Weapon_1":
                if (XGTS_DataManager.PlayerDatas[0].Weapon_Secondary) {
                    XGTS_GameManager.Instance.player.SetGun(XGTS_DataManager.PlayerDatas[0].Weapon_Secondary);
                }
                break;
            case "Pistol":
                if (XGTS_DataManager.PlayerDatas[0].Weapon_Pistol) {
                    XGTS_GameManager.Instance.player.SetGun(XGTS_DataManager.PlayerDatas[0].Weapon_Pistol);
                }
                break;
            case "MeleeWeapon":
            ...
            case "BackpackButton":
                if (XGTS_LvManager.Instance.MapName == "特勤处") {
                    ...
                    XGTS_UIManager.Instance.ShowPanel(XGTS_Constant.Panel.InventoryPanel, [null]);
                }
                break;
    ...
    protected onEnable(): void {
        EventManager.Scene.on(XGTS_Constant.Event.SHOW_INTERACT_BUTTON, this.ShowInteractButton, this);
    ...
    protected onDisable(): void {
        EventManager.Scene.off(XGTS_Constant.Event.SHOW_INTERACT_BUTTON, this.ShowInteractButton, this);
