...
@ccclass('XGTS_UIManager')
export class XGTS_UIManager extends Component {
    @property(JsonAsset)
    SceneDate: JsonAsset | null = null;
    private _panelDict: any = {}
    private _loadingPanelDict: any = {}
    private _arrPopupDialog: any = []
    private static _instance: XGTS_UIManager;
    public static get Instance() {
        if (!this._instance) {
            this._instance = new XGTS_UIManager();
        }
    ...
    public ShowPanel(panelPath: string, args?: any, cb?: Function) {
        ...
        let scriptName = `XGTS_` + panelPath.slice(idxSplit + 1);
    ...
    */
    ...
    public static ShowBroadcast(content: string, path: string, position: Vec2, delay: number = 0.75, tweenType: number = 2) {
        // XGTS_PoolManager.GetNodeByBundle(GameManager.GameData.DefaultBundle, XGTW_Constant.Path.Broadcast, find("Canvas")).then(node => node.getComponent(XGTW_Broadcast).Show(content, path, position, delay, tweenType));
    }
}