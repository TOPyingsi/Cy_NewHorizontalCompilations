...
@ccclass('XGTS_InventoryPanel_Double')
export default class XGTS_InventoryPanel_Double extends PanelBase {
    playerNum :number = 0;
    PlayerNumlbL:Label = null;
    Content: Node = null;
    ...
    protected onLoad(): void {
        this.PlayerNumlbL = NodeUtil.GetComponent("PlayerNum", this.node, Label);
    ...
    Show(spawnInverntory: Function,playerNum:number): void {
        ...
        this.PlayerNumlbL.string ="P" + playerNum.toString();
    ...
    OnDragEnd(item: XGTS_Item, point: Vec2) {
        ...
        EventManager.Scene.emit(XGTS_Constant.Event.REFRESH_EUIP + "_" + this.playerNum);
    ...
    OnButtonClick(event: Event) {
        ...
        const showItemInfoPanel = (container: any, key: string, content: XGTS_WeaponContent) => {
            ...
            XGTS_UIManager.Instance.ShowPanel(XGTS_Constant.Panel.ItemInfoPanel, [data, true, (option: string) => {
                if (option == "Sell") {
                    ...
                    container[key] = null;
                    if (XGTS_GameManager.Instance.players[this.playerNum-1].weapon == data) XGTS_GameManager.Instance.players[this.playerNum-1].weapon = null;
                    EventManager.Scene.emit(XGTS_Constant.Event.REFRESH_EUIP + "_" + this.playerNum);
                    EventManager.Scene.emit(XGTS_Constant.Event.REFRESH_GAME_ITEM_UI);
                }

                if (option == "Takeoff") {
                    if (XGTS_DataManager.PlayerDatas[1].AddItemToInventory(data)) {
                        container[key] = null;
                        if (XGTS_GameManager.Instance.players[this.playerNum-1].weapon == data) XGTS_GameManager.Instance.players[this.playerNum-1].weapon = null;
                        EventManager.Scene.emit(XGTS_Constant.Event.REFRESH_EUIP + "_" + this.playerNum);
                        EventManager.Scene.emit(XGTS_Constant.Event.REFRESH_GAME_ITEM_UI);
        ...
        switch (event.target.name) {
            ...
            case "Weapon_BodyArmor":
                ...
                break;
            case "ReturnButton":
                XGTS_UIManager.Instance.HidePanel(XGTS_Constant.Panel.InventoryPanel_Double);
                break;
        }
    }
