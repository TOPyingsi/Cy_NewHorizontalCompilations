...
@ccclass('XGTS_InventoryPanel')
export default class XGTS_InventoryPanel extends PanelBase {
    Content: Node = null;
    ...
    protected start(): void {
        ...
        for (let i = 0; i < this.PocketInventorys.children.length; i++) {
            ...
            if (!XGTS_DataManager.PlayerDatas[0].PocketData[i]) {
                XGTS_DataManager.PlayerDatas[0].PocketData[i] = [];
            }
            singleInventory.InitSingle(XGTS_DataManager.PlayerDatas[0].PocketData[i], XGTS_DataManager.PlayerDatas[0].SingleGrids[i]);
    ...
    RefreshMoney() {
        this.MoneyLabel.string = `${XGTS_DataManager.PlayerDatas[0].Money.toLocaleString()}`;
    }

    RefreshTotalValue() {
        let totalValue = 0;
        //装备
        if (XGTS_DataManager.PlayerDatas[0].Weapon_Primary) totalValue += XGTS_DataManager.PlayerDatas[0].Weapon_Primary.Price;
        if (XGTS_DataManager.PlayerDatas[0].Weapon_Secondary) totalValue += XGTS_DataManager.PlayerDatas[0].Weapon_Secondary.Price;
        if (XGTS_DataManager.PlayerDatas[0].Weapon_Pistol) totalValue += XGTS_DataManager.PlayerDatas[0].Weapon_Pistol.Price;
        if (XGTS_DataManager.PlayerDatas[0].Weapon_Helmet) totalValue += XGTS_DataManager.PlayerDatas[0].Weapon_Helmet.Price;
        if (XGTS_DataManager.PlayerDatas[0].Weapon_BodyArmor) totalValue += XGTS_DataManager.PlayerDatas[0].Weapon_BodyArmor.Price;

        //口袋
        if (XGTS_DataManager.PlayerDatas[0].PocketData) {
            for (let i = 0; i < XGTS_DataManager.PlayerDatas[0].PocketData.length; i++) {
                const element = XGTS_DataManager.PlayerDatas[0].PocketData[i];
                if (element[0]) totalValue += element[0].Price;
            }
        }

        //胸挂
        if (XGTS_DataManager.PlayerDatas[0].ChestRigData) {
            totalValue += XGTS_DataManager.PlayerDatas[0].ChestRigData.Price;
            for (let i = 0; i < XGTS_DataManager.PlayerDatas[0].ChestRigData.EquipData.ItemData.length; i++) {
                let data = XGTS_DataManager.PlayerDatas[0].ChestRigData.EquipData.ItemData;
        ...
        //背包
        if (XGTS_DataManager.PlayerDatas[0].BackpackData) {
            totalValue += XGTS_DataManager.PlayerDatas[0].BackpackData.Price;
            for (let i = 0; i < XGTS_DataManager.PlayerDatas[0].BackpackData.EquipData.ItemData.length; i++) {
                let data = XGTS_DataManager.PlayerDatas[0].BackpackData.EquipData.ItemData;
                for (let j = 0; j < data.length; j++) {
                    if (data[j]) totalValue += data[j].Price;
                }
            }
        ...
        //安全箱
        if (XGTS_DataManager.PlayerDatas[0].SafeBox) {
            for (let i = 0; i < XGTS_DataManager.PlayerDatas[0].SafeBox.ItemData.length; i++) {
                let data = XGTS_DataManager.PlayerDatas[0].SafeBox.ItemData;
                for (let j = 0; j < data.length; j++) {
                    if (data[j]) totalValue += data[j].Price;
                }
            }
        ...
        this.TotalValueLabel.string = "总价值：" + XGTS_DataManager.PlayerDatas[0].GetTotalValue().toLocaleString();
    }
    ...
    OnButtonClick(event: Event) {
        ...
        const showItemInfoPanel = (container: any, key: string, content: XGTS_WeaponContent) => {
            ...
            XGTS_GameManager.itemPlayerNum = 0;
            XGTS_UIManager.Instance.ShowPanel(XGTS_Constant.Panel.ItemInfoPanel, [data, true, (option: string) => {
                ...
                if (option == "Takeoff") {
                    if (XGTS_DataManager.PlayerDatas[0].AddItemToInventory(data)) {
                        container[key] = null;
                        if (XGTS_GameManager.Instance.player.weapon == data) XGTS_GameManager.Instance.player.weapon = null;
                        EventManager.Scene.emit(XGTS_Constant.Event.REFRESH_EUIP);
                        EventManager.Scene.emit(XGTS_Constant.Event.REFRESH_GAME_ITEM_UI);
        ...
        switch (event.target.name) {
            case "Weapon_Primary":
                if (XGTS_DataManager.PlayerDatas[0].Weapon_Primary) {
                    showItemInfoPanel(XGTS_DataManager.PlayerDatas[0], "Weapon_Primary", this.Weapon_Primary);
                }
                break;
            case "Weapon_Pistol":
                if (XGTS_DataManager.PlayerDatas[0].Weapon_Pistol) {
                    showItemInfoPanel(XGTS_DataManager.PlayerDatas[0], "Weapon_Pistol", this.Weapon_Pistol);
                }
                break;
            case "Weapon_Secondary":
                if (XGTS_DataManager.PlayerDatas[0].Weapon_Secondary) {
                    showItemInfoPanel(XGTS_DataManager.PlayerDatas[0], "Weapon_Secondary", this.Weapon_Secondary);
                }
                break;
            case "Weapon_Helmet":
                if (XGTS_DataManager.PlayerDatas[0].Weapon_Helmet) {
                    showItemInfoPanel(XGTS_DataManager.PlayerDatas[0], "Weapon_Helmet", this.Weapon_Helmet);
                }
                break;
            case "Weapon_BodyArmor":
                if (XGTS_DataManager.PlayerDatas[0].Weapon_BodyArmor) {
                    showItemInfoPanel(XGTS_DataManager.PlayerDatas[0], "Weapon_BodyArmor", this.Weapon_BodyArmor);
                }
                break;
            case "ReturnButton":
