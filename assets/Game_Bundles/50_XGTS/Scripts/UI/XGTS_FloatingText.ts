...
@ccclass('XGTS_FloatingText')
export class XGTS_FloatingText extends Component {
    ...
    ShowTween1(delay: number) {
        ...
        tween(this.node)
            ...
            .call(() => XGTS_PoolManager.Instance.Put(this.node))
            .start();
    }
    ShowTween2(delay: number) {
        tween(this.label.node.getComponent(UIOpacity)).delay(delay).to(0.25, { opacity: 0 }).start();
        tween(this.node).delay(delay).to(0.25, { position: v3(0, 250) }).call(() => { XGTS_PoolManager.Instance.Put(this.node) }).start();
    }
}