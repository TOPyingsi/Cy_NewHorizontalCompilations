...
@ccclass('XGTS_SafeBoxPanel')
export class XGTS_SafeBoxPanel extends PanelBase {
    Panel: Node = null;
    SafeBox: Node = null;
    ...
    Show(callback: Function) {
        XGTS_AudioManager.Instance.PlayBGM(XGTS_Audio.SafeBoxBG);
    ...
    OnButtonClick(event: Event) {
        ...
        switch (event.target.name) {
            case "Button":
                ...
                const wrongFlink = () => {
                    XGTS_AudioManager.Instance.PlaySFX(XGTS_Audio.SafeBoxF);
                    this.scheduleOnce(() => {
                        let color: Color[] = [];
                ...
                const sucessFlink = () => {
                    ...
                    this.scheduleOnce(() => {
                        ...
                        this.scheduleOnce(() => {
                            ...
                            count++;
                            if (count >= 5) {
                                XGTS_UIManager.Instance.HidePanel(XGTS_Constant.Panel.SafeBoxPanel);
                ...
                if (result) {
                    if (result.getComponent(Label).string == this.passworld[this.crossIndex]) {
                        ...
                        XGTS_AudioManager.Instance.PlaySFX(XGTS_Audio.SafeBoxT);
