...
@ccclass('XSHZ_AIControl')
export class XSHZ_AIControl extends Component {
    ...
    private Startforbid: number = 2;//AI开局禁止行动秒数
    start() {
        this.unit = this.node.getComponent(XSHZ_Unit);
        // 初始化最后玩家位置
        if (XSHZ_GameManager.Instance && XSHZ_GameManager.Instance.PlayerNode) {
            this.lastPlayerPosition = XSHZ_GameManager.Instance.PlayerNode.worldPosition.clone();
        }
        // 初始化战术状态
    ...
    update(dt: number) {
        this.Startforbid -= dt;
        if (!XSHZ_GameManager.Instance || !XSHZ_GameManager.Instance.PlayerNode || this.Startforbid > 0) {
            return;
        }
    ...
    private makeDecision() {
        const playerNode = XSHZ_GameManager.Instance.PlayerNode;
    ...
    private predictPlayerPosition(playerPosition: Vec3, playerNode: Node): Vec3 {
        const playerUnit = playerNode.getComponent(XSHZ_Unit);
    ...
    private executeFlankingBehavior(predictedPlayerPosition: Vec3, distanceToPlayer: number) {
        ...
        if (distanceToPlayer > this.attackRange * 2) {
            ...
            if (this.unit.SkillCount > 0) {
                this.unit.TongLing(XSHZ_GameManager.SkillData[XSHZ_GameManager.EnemyID])
                return;
            }
            if (currentTime - this.lastAttackTime > this.attackCooldown) {
                this.unit.AttackClick();
