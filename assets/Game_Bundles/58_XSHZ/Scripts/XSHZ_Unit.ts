...
import { XSHZ_AttackBox } from './XSHZ_AttackBox';
import { XSHZ_GameManager } from './XSHZ_GameManager';
import { XSHZ_Bullet } from './XSHZ_Bullet';
...
import { XSHZ_TongLing } from './XSHZ_TongLing';
const { ccclass, property } = _decorator;

@ccclass('XSHZ_Unit')
export class XSHZ_Unit extends Component {
    ...
    //读取角色数据
    LoadUnitData() {
        let data = XSHZ_Constant.GetUnitDataByName(this.node.name);
    ...
    SkillClick(num: number): void {
        ...
        this.Mp -= num * 20;//MP消耗
        director.getScene().emit(XSHZ_EasyControllerEvent.释放技能);
    ...
    TongLing(num: number) {
        ...
        director.getScene().emit(XSHZ_EasyControllerEvent.消耗技能豆);
        XSHZ_incident.Loadprefab("PreFab/通灵/" + num).then((pre: Prefab) => {
            let TL = instantiate(pre);
            TL.setParent(XSHZ_GameManager.Instance.BG);
            TL.setWorldPosition(this.node.worldPosition.clone());
            TL.getComponent(XSHZ_TongLing).Init(this.node, this.Attack, !this.orientation);
        });
    }
    ...
    AttackCollisionLook(name: string) {
        ...
        collider.getComponent(XSHZ_AttackBox).Attack = this.Attack * this.GetSkillScale(name);
        collider.getComponent(XSHZ_AttackBox).Attacknode = this.node;
        collider.getComponent(XSHZ_AttackBox).Show();
    ...
    Hurt(attack: number): boolean {
        ...
        this.Hp -= attack;
        this.Mp += 3;
        director.getScene().emit(XSHZ_EasyControllerEvent.BeatBack, this.node, attack);
        ...
        XSHZ_incident.Loadprefab("PreFab/击中特效").then((pre: Prefab) => {
    ...
    private handleHitEffect() {
        ...
        // 播放受击动画
        this.aniName = "受击";
        director.getScene().emit(XSHZ_EasyControllerEvent.受击, this._comboCount);
        // 判断是否浮空
    ...
    die() {
        ...
        if (this.node.getComponent(XSHZ_AIControl)) this.node.getComponent(XSHZ_AIControl).enabled = false;
        if (this.node.getComponent(XSHZ_PlayerControl)) this.node.getComponent(XSHZ_PlayerControl).enabled = false;
        this.StopMove();
        // 1秒后销毁节点
        this.scheduleOnce(() => {
            director.getScene().emit(XSHZ_EasyControllerEvent.角色死亡, this.node.getComponent(XSHZ_AIControl) ? true : false);
    ...
    GenerateBullet(id: number, pos: Vec3, attack: number) {
        ...
        nd.active = false;
        nd.setParent(XSHZ_GameManager.Instance.BG);
        nd.setWorldPosition(this.node.getWorldPosition().clone().add(pos));
        nd.active = true;
        nd.getComponent(XSHZ_Bullet).Init(this.node, attack, !this.orientation);
    }
    //角色位移
    MovePos(pos: Vec2, Time: number = 0.1) {
        ...
        if (pos.y > XSHZ_GameManager.Instance.MapData.x) pos.y = XSHZ_GameManager.Instance.MapData.x;
        if (pos.y < XSHZ_GameManager.Instance.MapData.y) pos.y = XSHZ_GameManager.Instance.MapData.y;
        if (pos.x < XSHZ_GameManager.Instance.MapData.z) pos.x = XSHZ_GameManager.Instance.MapData.z;
        if (pos.x > XSHZ_GameManager.Instance.MapData.w) pos.x = XSHZ_GameManager.Instance.MapData.w;
