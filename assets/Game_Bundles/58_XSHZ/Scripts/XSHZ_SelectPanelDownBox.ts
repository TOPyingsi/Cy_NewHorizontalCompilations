...
@ccclass('XSHZ_SelectPanelDownBox')
export class XSHZ_SelectPanelDownBox extends Component {
    @property()
    public ID: number = 0;
    ...
    protected start(): void {
        director.getScene().on(XSHZ_EasyControllerEvent.选中角色, this.OnPlayerSelect, this);
        director.getScene().on(XSHZ_EasyControllerEvent.角色选择框选中, (ID: number) => {
            if (ID == this.ID) {
                this.IsPitchOn = true;
    ...
    OnClick() {
        ...
        director.getScene().emit(XSHZ_EasyControllerEvent.选择界面切换页面, "忍者");
        director.getScene().emit(XSHZ_EasyControllerEvent.角色选择框选中, this.ID);
    }

    //有角色被选中
    OnPlayerSelect(name: string) {
        if (this.IsPitchOn) {
            XSHZ_GameManager.TeamData[this.ID] = name;
            this.Name = name;
            this.Show();
        }
    }
    ...
    //根据选择的角色刷新当前的图像
    Show() {
        XSHZ_incident.LoadSprite("Sprite/头像/" + this.Name).then((sprite: SpriteFrame) => {
