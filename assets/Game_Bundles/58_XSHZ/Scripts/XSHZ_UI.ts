...
@ccclass('XSHZ_UI')
export class XSHZ_UI extends Component {

    private FindNode: XSHZ_Unit = null;//UI追寻单位
    ...
    start() {
        ...
        director.getScene().on(XSHZ_EasyControllerEvent.隐藏加载界面, () => { this.node.getChildByName("加载界面").active = false; })
        if (XSHZ_GameManager.Instance.LoadIndex >= XSHZ_GameManager.Instance.LoadMaxIndex) {
        ...
        director.getScene().on(XSHZ_EasyControllerEvent.主程序就绪, this.InitSkillSprite, this)
        director.getScene().on(XSHZ_EasyControllerEvent.弹出结算窗口, this.OpenGameOverPanel, this)
        director.getScene().on(XSHZ_EasyControllerEvent.受击, this.AttackUI, this)
        director.getScene().on(XSHZ_EasyControllerEvent.PlayerOnLoad, (nd: Node) => { this.FindNode = nd.getComponent(XSHZ_Unit); })
        director.getScene().on(XSHZ_EasyControllerEvent.弹出下一场, this.OpenNextUI, this)
    ...
    //初始化技能图片
    InitSkillSprite() {
        let name = XSHZ_GameManager.TeamData[XSHZ_GameManager.PlayerID];
        XSHZ_incident.LoadSprite("Sprite/技能图片/" + name + "/0").then((sp: SpriteFrame) => {
            this.node.getChildByName("技能1").getComponent(Sprite).spriteFrame = sp;
        })
        XSHZ_incident.LoadSprite("Sprite/技能图片/" + name + "/1").then((sp: SpriteFrame) => {
            this.node.getChildByName("技能2").getComponent(Sprite).spriteFrame = sp;
        })
        XSHZ_incident.LoadSprite("Sprite/技能图片/" + name + "/2").then((sp: SpriteFrame) => {
            this.node.getChildByName("技能3").getComponent(Sprite).spriteFrame = sp;
        })
        XSHZ_incident.LoadSprite("Sprite/通灵头像/" + XSHZ_GameManager.SkillData[XSHZ_GameManager.PlayerID]).then((sp: SpriteFrame) => {
    ...
    OnButtomClick(btn: EventTouch) {
        switch (btn.target.name) {
            case "攻击":
                director.getScene().emit(XSHZ_EasyControllerEvent.ATTACK);
                break;
            case "技能1":
                director.getScene().emit(XSHZ_EasyControllerEvent.SKILL, 1);
                break;
            case "技能2":
                director.getScene().emit(XSHZ_EasyControllerEvent.SKILL, 2);
                break;
            case "技能3":
                director.getScene().emit(XSHZ_EasyControllerEvent.SKILL, 3);
                break;
            case "通灵":
                director.getScene().emit(XSHZ_EasyControllerEvent.TongLing);
                break;
            case "退出":
                if (!Banner.TimeMask) {
    ...
    OpenGameOverPanel(IsWinner: boolean) {
        ...
        let money: number = 0;
        let exp: number = 0;
        if (XSHZ_GameManager.GameMode == "1V1") {
            if (IsWinner) {
                money = 50;
                exp = 30;
            } else {
        ...
        if (XSHZ_GameManager.GameMode == "无尽试炼") {
            money = XSHZ_GameManager.WinNum * 25;
            exp = XSHZ_GameManager.WinNum * 15;
        }
        if (XSHZ_GameManager.GameMode == "强者挑战") {
            if (IsWinner) {
                if (XSHZ_GameManager.difficulty == "困难") {
                    money = XSHZ_GameManager.WinNum * 150;
                    exp = XSHZ_GameManager.WinNum * 150;
                }
                if (XSHZ_GameManager.difficulty == "极难") {
                    money = XSHZ_GameManager.WinNum * 300;
                    exp = XSHZ_GameManager.WinNum * 300;
                }
            } else {
                money = 20;
                exp = 10;
        ...
        XSHZ_GameData.Instance.Money += money;
        XSHZ_GameData.Instance.GameData[0] += exp;
