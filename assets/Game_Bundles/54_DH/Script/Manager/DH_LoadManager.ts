 ...
 */
@ccclass('DH_LoadManager')
export class DH_LoadManager extends Component {
    public static Instance: DH_LoadManager;  // 单例实例，全局访问点
     ...
     */
    onLoad() {
        DH_LoadManager.Instance = this;  // 设置单例实例
     ...
     */
    getResource(resourceId: string, type: DH_ResourceType, callback: (resource: any) => void) {
        // 生成唯一缓存键
        ...
        // 加载资源
        BundleManager.GetBundle('54_DH').load(path, resourceType, (err, asset) => {
            if (err) {
     ...
     * @returns 唯一缓存键字符串
     */
    private generateCacheKey(resourceId: string, type: DH_ResourceType): string {
     ...
     */
    private getResourceInfo(type: DH_ResourceType, resourceId: string): { path: string, resourceType: any } {
        switch (type) {
            case DH_ResourceType.MapIcon:
                // 图标资源路径格式: icons/[id]
                return {
    ...
    getMapIconById(mapId: string, callback: (spriteFrame: SpriteFrame | null) => void) {
        this.getResource(mapId, DH_ResourceType.MapIcon, (res) => {
    ...
    getMapBgById(mapId: string, callback: (spriteFrame: SpriteFrame | null) => void) {
        this.getResource(mapId, DH_ResourceType.MapBg, (res) => {
            callback(res as SpriteFrame | null);
        });
    }
    ...
    getAnglerIconById(anglerId: string, callback: (spriteFrame: SpriteFrame | null) => void) {
        this.getResource(anglerId, DH_ResourceType.AnglerIcon, (res) => {
            callback(res as SpriteFrame | null);
        });
    }
    ...
    getFishingRodIconById(fishingRodId: string, callback: (spriteFrame: SpriteFrame | null) => void) {
        this.getResource(fishingRodId, DH_ResourceType.FishingRodIcon, (res) => {
            callback(res as SpriteFrame | null);
        });
    }
    ...
    getSpecialItemIconById(specialItemId: string, callback: (spriteFrame: SpriteFrame | null) => void) {
        this.getResource(specialItemId, DH_ResourceType.SpecialItemIcon, (res) => {
            callback(res as SpriteFrame | null);
        });
    }
    ...
    getFishIconById(fishId: string, callback: (spriteFrame: SpriteFrame | null) => void) {
        this.getResource(fishId, DH_ResourceType.FishIcon, (res) => {
            callback(res as SpriteFrame | null);
        });
    }
    ...
    getSkillIconById(skillId: string, callback: (spriteFrame: SpriteFrame | null) => void) {
        this.getResource(skillId, DH_ResourceType.SkillIcon, (res) => {
            callback(res as SpriteFrame | null);
        });
    }
    ...
    getMapPrefabById(mapId: string, callback: (prefab: Prefab | null) => void) {
        this.getResource(mapId, DH_ResourceType.MapPrefab, (res) => {
    ...
    getAnglerPrefabById(anglerId: string, callback: (prefab: Prefab | null) => void) {
        let cacheKey = this.generateCacheKey("钓友_0", DH_ResourceType.AnglerPrefab)
        this.resourceCache[cacheKey] = null
        this.getResource("钓友_0", DH_ResourceType.AnglerPrefab, (res) => {
            callback(res as Prefab | null);
        });
    }
    ...
    getFishPrefabById(fishId: string, callback: (prefab: Prefab | null) => void) {
        this.getResource(fishId, DH_ResourceType.FishPrefab, (res) => {
            callback(res as Prefab | null);
        });
    }
     ...
     * @param type 资源类型
     */
    releaseResource(resourceId: string, type: DH_ResourceType) {
        const cacheKey = this.generateCacheKey(resourceId, type);
        if (this.resourceCache[cacheKey]) {
            // 释放资源引用
