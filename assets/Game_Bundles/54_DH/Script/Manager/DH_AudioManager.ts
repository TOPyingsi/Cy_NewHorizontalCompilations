...
@ccclass('DH_AudioManager')
export class DH_AudioManager extends Component {
    private static instance: DH_AudioManager;
    ...
    // 单例模式
    public static getInstance(): DH_AudioManager {
        if (!DH_AudioManager.instance) {
            // 查找场景中是否已存在GameManager节点
            const existingNode = director.getScene().getChildByName('GameManager');
            if (existingNode) {
                DH_AudioManager.instance = existingNode.getComponent(DH_AudioManager);
            } else {
                // 如果不存在，则创建新节点并添加组件
                const gameManagerNode = new Node('GameManager');
                director.getScene().addChild(gameManagerNode);
                DH_AudioManager.instance = gameManagerNode.addComponent(DH_AudioManager);
            }
        }
        return DH_AudioManager.instance;
    }

    protected onLoad(): void {
        DH_AudioManager.instance = this;
