...
@ccclass('SJZGMMT_SkinBox')
export class SJZGMMT_SkinBox extends Component {
    ...
    Init(Name: string, skinPanel: Node) {
        ...
        this.SkinName = Name;
        this.SkinData = SJZGMMT_Constant.getSkinDataByName(Name);
        SJZGMMT_Incident.LoadSprite("Sprites/皮肤/" + this.SkinData.Quality).then((sp: SpriteFrame) => {
            this.node.getChildByName("Bg").getComponent(Sprite).spriteFrame = sp;
        });
        SJZGMMT_Incident.LoadSprite("Sprites/皮肤/皮肤图像/" + this.SkinData.Name).then((sp: SpriteFrame) => {
            this.node.getChildByName("图").getComponent(Sprite).spriteFrame = sp;
        });
        SJZGMMT_Incident.LoadSprite("Sprites/皮肤/文本/" + this.SkinData.Quality).then((sp: SpriteFrame) => {
