import { _decorator, Color, Component, director, Node, Sprite, tween } from 'cc';
...
@ccclass('SJZGMMT_vessel')
export class SJZGMMT_vessel extends Component {
    ...
    protected start(): void {
        this.Init();
        director.getScene().on(SJZGMMT_EventManager.进入容器范围, this.drawOutline, this);
        director.getScene().on(SJZGMMT_EventManager.离开容器范围, this.drawOutline2, this);
    }

    Init() {
        // 获取当前场景的爆率`
        const sceneData = SJZGMMT_Constant.GetSceneDataByName(SJZGMMT_GameManager.GameScene);
        const sceneDetonation = sceneData ? sceneData.场景爆率 : 0;
        //研究所爆率
        let studyDetonation = SJZGMMT_Constant.LaboratoryLevelData[2][SJZGMMT_GameData.Instance.LaboratoryLevel[2]];
        // 计算最终爆率值
        const finalDetonation = this.qualitydetonation + sceneDetonation + studyDetonation + SJZGMMT_GameManager.Instance.ExtraDetonation;
        ...
        // 获取所有回收物列表
        const allRecycleProps = SJZGMMT_Constant.getAllPropByType(SJZGMMT_PropType.回收物);

        // 将回收物按品质分组
        const propsByQuality: { [key: number]: string[] } = {};
        for (const propName of allRecycleProps) {
            const propData = SJZGMMT_Constant.getPropDataByName(propName);
            if (propData) {
        ...
        for (let i = 0; i < propCount; i++) {
            ...
            // 根据品质选择道具
            if (selectedQuality === SJZGMMT_Quality.红色 && this.RedPropList.length > 0) {
                ...
                // 确保选中的道具是回收物类型且是红色品质
                const propData = SJZGMMT_Constant.getPropDataByName(selectedPropName);
                if (!propData || propData.type !== SJZGMMT_PropType.回收物 || propData.quality !== SJZGMMT_Quality.红色) {
    ...
    public getQualityCount(vesselData: { Name: string, Isobserve: boolean }[]): { [quality: number]: number } {
        ...
        // 初始化所有品质的计数为0
        for (const qualityKey in SJZGMMT_Quality) {
    ...
    // 计算各品质的权重，基于爆率
    private calculateQualityWeights(finalDetonation: number): { quality: SJZGMMT_Quality, weight: number }[] {
        // 将爆率标准化为0-1之间的值
        ...
        return [
            { quality: SJZGMMT_Quality.白色, weight: whiteWeight },
            { quality: SJZGMMT_Quality.绿色, weight: greenWeight },
            { quality: SJZGMMT_Quality.蓝色, weight: blueWeight },
            { quality: SJZGMMT_Quality.紫色, weight: purpleWeight },
            { quality: SJZGMMT_Quality.金色, weight: goldWeight },
            { quality: SJZGMMT_Quality.红色, weight: redWeight },
            { quality: SJZGMMT_Quality.炫彩, weight: rainbowWeight }
        ];
    }
    // 根据权重选择品质
    private selectQualityByWeight(weights: { quality: SJZGMMT_Quality, weight: number }[]): SJZGMMT_Quality {
        // 计算总权重
    ...
    public ShowActive() {
        ...
        tween(this.node.getChildByName("图片").getComponent(Sprite))
