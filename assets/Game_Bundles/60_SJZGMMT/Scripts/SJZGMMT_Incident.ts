...
@ccclass('SJZGMMT_Incident')
export class SJZGMMT_Incident extends Component {

    public static LoadSprite(Path: string) {
        return new Promise((resolve, reject) => {
            BundleManager.GetBundle("60_SJZGMMT").load(Path + "/spriteFrame", SpriteFrame, (err, data) => {
                if (err) {
    ...
    public static LoadTexture2D(Path: string) {
        return new Promise((resolve, reject) => {
            BundleManager.GetBundle("60_SJZGMMT").load(Path + "/texture", Texture2D, (err, data) => {
                if (err) {
                    console.log("没有找到图片" + Path);
                    return;
                }
    ...
    public static Loadprefab(Path: string) {
        return new Promise((resolve, reject) => {
            BundleManager.GetBundle("60_SJZGMMT").load(Path, Prefab, (err, data) => {
                if (err) {
    ...
    public static GetPropValue(PropData: string[]): number {
        ...
        for (let i = 0; i < PropData.length; i++) {
            value += SJZGMMT_Constant.getPropDataByName(PropData[i]).price;
        }
        return value;
    }
}
