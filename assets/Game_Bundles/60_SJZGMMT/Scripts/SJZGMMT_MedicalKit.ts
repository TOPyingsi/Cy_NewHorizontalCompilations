...
@ccclass('SJZGMMT_MedicalKit')
export class SJZGMMT_MedicalKit extends Component {
    start() {
        this.Show();
    }
    ...
    OnButtonClick(event: EventTouch) {
        ...
        if (SJZGMMT_GameManager.Instance.MedicalKit[id] > 0) {
            SJZGMMT_GameManager.Instance.MedicalKit[id]--;
            director.getScene().emit(SJZGMMT_EventManager.主角使用血包, id);
            this.Cd = 5;
        } else {
    ...
    Show() {
        this.node.children.forEach((element, index) => {
            if (SJZGMMT_GameManager.Instance.MedicalKit[index] > 0) {
                element.getChildByName("视频角标").active = false;
                element.getChildByName("次数").active = true;
                element.getChildByPath("次数/数量").getComponent(Label).string = `${SJZGMMT_GameManager.Instance.MedicalKit[index]}`;
            } else {
