...
@ccclass('SJZGMMT_PoolManager')
export class SJZGMMT_PoolManager extends Component {
    @property({ type: [Prefab] })
    Prefabs: Prefab[] = [];
    private static _instance: SJZGMMT_PoolManager;

    private _pools: Map<string, NodePool> = new Map(); // 对象池存储
    private _prefabs: Map<string, Prefab> = new Map(); // 预制体资源存储

    public static get Instance(): SJZGMMT_PoolManager {
        if (!this._instance) {
            this._instance = new SJZGMMT_PoolManager();
        }
        return this._instance;
    }
    protected onLoad(): void {
        SJZGMMT_PoolManager._instance = this;
    }
    protected start(): void {
        director.addPersistRootNode(this.node);
        this.Prefabs.forEach(prefab => {
            this.Preload(prefab, 10);
        });
     ...
     * @param parent 父节点（可选）
     */
    Get(prefabOrName: Prefab | string, parent?: Node, Worldposition?: Vec3): Node | null {
