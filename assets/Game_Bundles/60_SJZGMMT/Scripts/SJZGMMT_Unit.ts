...
import { SJZGMMT_Constant, SJZGMMT_WeaponItem } from './SJZGMMT_Constant';
...
import { SJZGMMT_DamageTip } from './SJZGMMT_DamageTip';
const { ccclass, property } = _decorator;

@ccclass('SJZGMMT_Unit')
export class SJZGMMT_Unit extends Component {
    ...
    public EnemyArray: SJZGMMT_Unit[] = [];
    public LockEnemy: SJZGMMT_Unit = null;
    ...
    public WeaponData: SJZGMMT_WeaponItem = null;
    public WeaponNode: Node = null;
    public Weapon: SJZGMMT_I_weapon = null;
    ...
    protected update(dt: number): void {
        ...
        if (this.FindModle == 0) {
            if (this.LockEnemy && this.LockEnemy.node?.isValid && this.LockEnemy.Hp > 0 && this.IsGunAttack && this.WeaponData.武器类型 == "远程") {
                ...
                this.WeaponNode.getComponent(SJZGMMT_I_weapon).SetWeaponRotation(this.LockEnemy.node.worldPosition);
            } else {
                this.LockEnemyState = false;
            }
        } else {
            //通过角度控制
            this.WeaponNode.getComponent(SJZGMMT_I_weapon).SetWeaponRotationToAngle(this.GunAngle);
        }
    ...
    protected start(): void {
        ...
        SJZGMMT_GameManager.Instance.UnitArray.push(this);
        ...
        director.getScene().on(SJZGMMT_EventManager.单位死亡, (nd: Node) => {
    ...
    public FindLockEnemy() {
        ...
        // 找到距离最近的敌人
        let closestEnemy: SJZGMMT_Unit = null;
    ...
    public FindEnemy() {
        this.EnemyArray = [];
        for (let i = 0; i < SJZGMMT_GameManager.Instance.UnitArray.length; i++) {
            if (SJZGMMT_GameManager.Instance.UnitArray[i].Camp != this.Camp) {
                this.EnemyArray.push(SJZGMMT_GameManager.Instance.UnitArray[i]);
            }
        }
    }
    ...
    public EquipWeapon(Name: string) {
        this.WeaponData = SJZGMMT_Constant.getWeaponDataByName(Name);
        SJZGMMT_Incident.Loadprefab("Prefabs/武器/" + this.WeaponData.Name).then((prefab: Prefab) => {
            let wp = instantiate(prefab);
            wp.setParent(this.node.getChildByName("武器"));
            // wp.setPosition(v3(0, 0, 0));
            this.WeaponNode = wp;
            this.Weapon = wp.getComponent(SJZGMMT_I_weapon);
            this.WeaponAnimationNode = wp.getChildByName("动画");
            //设置武器伤害((攻击力+武器伤害)/10)
            this.Weapon.SetAttack((this.Attack + SJZGMMT_Constant.getPropDataByName(this.WeaponData.Name).property) / 3);
    ...
    FIRE_START(Havedirection: boolean, direction: Vec3 = v3(0, 0, 0)) {
        ...
        if (this.WeaponData.武器类型 == "近战") {
            this.WeaponNode.getComponent(SJZGMMT_I_weapon).Attack();
        }
    ...
    FIRE_STOP() {
        ...
        this.WeaponNode.getComponent(SJZGMMT_I_weapon).StopAttack();
    }
    ...
    public TakeDamage(Damage: number) {
        ...
        Damage *= (1 - this.Defensive / (this.Defensive + 500));
        Damage = Math.floor(Damage);
        this.Hp -= Damage;
        let nd = SJZGMMT_PoolManager.Instance.Get("伤害显示图");
        nd.getComponent(SJZGMMT_DamageTip).Show(Damage, this.node.worldPosition);
        // console.log("受到伤害！！！");
        if (this.Hp <= 0) {
            SJZGMMT_AudioManager.globalAudioPlay("死亡音效");
            if (this.DieIsDes) {
                if (this.Camp != 0) {
                    SJZGMMT_GameManager.KillEnemy++;
                }
                director.getScene().emit(SJZGMMT_EventManager.单位死亡, this.node);
            }
        } else {
            SJZGMMT_AudioManager.globalAudioPlay("受击");
            this.HitFlash();//受击特效
        }
    }
