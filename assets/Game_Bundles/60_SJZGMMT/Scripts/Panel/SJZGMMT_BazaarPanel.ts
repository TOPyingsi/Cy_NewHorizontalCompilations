...
import { SJZGMMT_ShoppingBigBox } from '../SJZGMMT_ShoppingBigBox';
import { SJZGMMT_ShoppingBox } from '../SJZGMMT_ShoppingBox';
import { SJZGMMT_AudioManager } from '../SJZGMMT_AudioManager';
import { SJZGMMT_EventManager } from '../SJZGMMT_EventManager';
const { ccclass, property } = _decorator;

@ccclass('SJZGMMT_BazaarPanel')
export class SJZGMMT_BazaarPanel extends PanelBase {
    public BazaarData: string[][] = [
        ["神·横刀", "神·青龙", "神·猩红射手", "神·天玑矛", "神·未来之光", "神·王者之耀"],
        ["洛阳铲", "弓弩", "AK47", "QBZ", "狙击步枪", "蓝调", "腐蚀丛林", "异星科技", "未来", "蝰蛇", "毒龙", "大黄蜂", "鎏金M4", "玩具"
    ...
    protected start(): void {
        //初始化神话武器
        SJZGMMT_Incident.Loadprefab("Prefabs/UI/神话武器框").then((data: Prefab) => {
            this.BazaarData[0].forEach((element, index) => {
                let nd = instantiate(data);
                nd.setParent(this.node.getChildByPath("框/神话武器框/Mask/Content"));
                nd.getComponent(SJZGMMT_ShoppingBigBox).Init(element, this.BazaarPrice[0][index]);
            });
        })
        //初始化其他框
        SJZGMMT_Incident.Loadprefab("Prefabs/UI/普通商店武器框").then((data: Prefab) => {
            this.BazaarData[1].forEach((element, index) => {
                let nd = instantiate(data);
                nd.setParent(this.node.getChildByPath("框/武器框/Mask/Content"));
                nd.getComponent(SJZGMMT_ShoppingBox).Init(element, this.BazaarPrice[1][index]);
            });
            this.BazaarData[2].forEach((element, index) => {
                let nd = instantiate(data);
                nd.setParent(this.node.getChildByPath("框/头盔框/Mask/Content"));
                nd.getComponent(SJZGMMT_ShoppingBox).Init(element, this.BazaarPrice[2][index]);
            });
            this.BazaarData[3].forEach((element, index) => {
                let nd = instantiate(data);
                nd.setParent(this.node.getChildByPath("框/防具框/Mask/Content"));
                nd.getComponent(SJZGMMT_ShoppingBox).Init(element, this.BazaarPrice[3][index]);
            });
        })
    }
    OnButtonClick(event: EventTouch) {
        ...
        switch (event.target.name) {
            case "关闭":
                SJZGMMT_UIManager.Instance.HidePanel(SJZGMMT_Constant.Panel.BazaarPanel);
                break;
            case "神话武器":
    ...
    ChanggeIndex(Name: string) {
        SJZGMMT_UIManager.Instance.SJZGMMT_Emit(SJZGMMT_EventManager.黑市切换栏位, Name);
