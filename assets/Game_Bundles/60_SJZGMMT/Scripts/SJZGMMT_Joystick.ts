import { _decorator, Component, Node, Event, Vec2, SystemEvent, systemEvent, Touch, Vec3, v2, macro, UITransform, KeyCode, EventTarget, EventTouch, input, Input, v3 } from 'cc';
import { EventManager } from 'db://assets/Scripts/Framework/Managers/EventManager';
import { SJZGMMT_EventManager } from './SJZGMMT_EventManager';
const { ccclass, property } = _decorator;

@ccclass('SJZGMMT_Joystick')
export class SJZGMMT_Joystick extends Component {
    ...
    OnTouchMove(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            ...
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                ...
                //     degree += 360;
                // }
                EventManager.Scene.emit(SJZGMMT_EventManager.MOVEMENT, v3(dirX, dirY, len / radius));
            }
        }
    }
    ...
    OnTouchEnd(event: EventTouch) {
        ...
        for (let i = 0; i < touches.length; ++i) {
            let touch = touches[i];
            if (this._movementTouch && touch.getID() == this._movementTouch.getID()) {
                EventManager.Scene.emit(SJZGMMT_EventManager.MOVEMENT_STOP);
    ...
    updateDirection() {
        ...
        EventManager.Scene.emit(SJZGMMT_EventManager.MOVEMENT, v3(this._dir.x, this._dir.y, 1.0));
    }
    //#endregion
}