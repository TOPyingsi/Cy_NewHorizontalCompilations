...
@ccclass('SJZGMMT_Knife')
export class SJZGMMT_Knife extends SJZGMMT_I_weapon {
    @property()
    ...
    onStartContact(selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
        if (otherCollider.node.getComponent(SJZGMMT_Unit)) {
            if (otherCollider.node.getComponent(SJZGMMT_Unit)?.Camp != this._camp) {
                otherCollider.node.getComponent(SJZGMMT_Unit).TakeDamage(this._attack / this._speed);
            }
        }
    }
